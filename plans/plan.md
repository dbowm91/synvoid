...
These items were fixed as part of the deferred items deep dive:

| ID | Issue | Fix | Commit |
|----|-------|-----|--------|
| **DNS-QUERY** | QueryCoalescer max_wait_ms unused | Async redesign: added max_wait Duration field, changed get_or_wait() to async with tokio::timeout | 4d126e9e |
| **SUP-1** | gRPC Control Plane TLS not implemented | Added control_api_tls config, tonic TLS support, --control-api-tls CLI flag | 4d126e9e |
| **BUG-PL-4** | macOS Seatbelt silent failure | Runtime detection via dlsym, proper error instead of silent Ok(()) | 4d126e9e |
| **PR-6** | ProxyHeadersConfig not passed through | Added proxy_headers_config field and builder method | d10cffff |
...
*All fixable deferred items completed. Only major architectural items remain.*
