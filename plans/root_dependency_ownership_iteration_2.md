...
| webpki-roots | synvoid-http-client | no | none (typed_pool removed in iteration 5) | Now only used by `crates/synvoid-http-client/src/lib.rs` as native cert fallback. Root no longer depends on it. |
