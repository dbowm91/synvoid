...
Milestone D accelerates warm CI performance and localized developer iteration through compiler output reuse (`sccache`), standardized Rust setup, and conservative affected-package selection with reverse-dependent closure.
...
| D2: Introduce sccache | Complete | sccache enabled for PR fast lane compilation jobs |
...
| D8: CI integration | Complete | `pr-fast.yml` updated with selector + sccache |
...
### Cache Architecture
- **4-layer cache model**: Cargo sources, tool binaries, compiler outputs (sccache), target metadata (rust-cache)
- **Cache policy**: `docs/testing/cache-policy.md` documents key dimensions, invalidation rules, size limits
- **sccache**: Compiler output reuse for eligible CI jobs (PR fast lane security-regression, guard-suite)
...
- **Branch protection**: Required checks (fmt, clippy, security-regression, guard-suite) always run
- **sccache stats**: Reported in CI summary for compilation-heavy jobs
...
- sccache infrastructure ready for measurement
