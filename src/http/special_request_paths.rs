...
#[cfg(feature = "mesh")]
use crate::http::request_parse::should_handle_key_exchange_path;
#[cfg(feature = "mesh")]
...
use crate::http::request_parse::parse_http01_challenge_token;
    ...
    if should_handle_key_exchange_path(path) {
        if let Some(ref mesh_cfg) = mesh_config {
                ...
                && mesh_cfg.origin_signing_key.is_some()
            {
                ...
                let state =
                    crate::mesh::passover_key_exchange::KeyExchangeHttpState::new(mesh_cfg.clone())
                        .with_transport(mesh_transport.clone());

                let response = if path == "/key-request-origin" && method == http::Method::POST {
                    match serde_json::from_slice::<
                        crate::mesh::passover_key_exchange::KeyRequestOriginHttp,
                    >(&body_bytes)
                    {
                        Ok(mut req_data) => {
                            ...
                            let result =
                                crate::mesh::passover_key_exchange::key_request_origin_http(
