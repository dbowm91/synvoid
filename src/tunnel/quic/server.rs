...
use crate::config::{TunnelQuicConfig, TunnelQuicPeerConfig, PortMappingConfig, VpnAccessLevel};
use crate::tunnel::quic::runtime::{QuicRuntime, QuicConnection, IncomingConnection};
use crate::tunnel::quic::tls::QuicTlsConfig;
use crate::tunnel::quic::messages::{PortMapping, DatagramCapabilities, DatagramMessage, TunnelMessage};
use crate::tunnel::quic::health::QuicHealthMonitor;
use crate::tunnel::quic::framing::{read_message, write_message};
use crate::tunnel::quic::registry::{QUIC_TUNNEL_REGISTRY, TunnelSessionInfo};
use crate::tunnel::quic::validation::{validate_client_id, validate_peer_id, validate_identifier, validate_port};
...
impl QuicTunnelServer {
    pub fn new(
        ...
        tracing::info!(
            ...
            max_auth_attempts, auth_window_secs
        );
    ...
    pub async fn run(&mut self) {
        ...
        tokio::spawn(async move {
            Self::connection_loop(connection_rx, sessions, config, shutdown_rx, connection_limit, runtime, auth_rate_limiter).await;
        });
        ...
        match msg {
            TunnelMessage::Hello { client_id, auth_token, mappings, .. } => {
                ...
                if !auth_rate_limiter.check_and_record(&client_id) {
                    ...
                    let error = TunnelMessage::AuthFailure {
                        reason: "Too many authentication attempts. Please try again later.".to_string(),
                    };
                    write_message(&mut send_stream, &error).await?;
                ...
                let registry_mappings: HashMap<String, u16> = session.mappings.iter()
                ...
                runtime.add_session(QuicConnection {
                    remote_addr,
                    peer_id: None,
                    session_id: session_id.clone(),
                    client_id: client_id.clone(),
                ...
                tracing::info!("QUIC session established: {} for client {} (access: {:?}, datagrams: {})", 
                    session_id, client_id, session.access_level, datagram_caps.supported);

                Self::session_loop(connection, session, sessions, max_message_size, runtime).await?;
            }
    ...
    async fn handle_stream(
        ...
        match msg {
            ...
            TunnelMessage::StreamOpen { identifier, port, protocol, tls_passthrough } => {
                tracing::debug!("Stream open request for {} ({}:{}) in session {} (tls_passthrough={})", identifier, protocol, port, session.id, tls_passthrough);
                ...
                if !session.can_access_port(port, &protocol) || !session.can_access_port(upstream_port, &protocol) {
                    tracing::warn!(
                    ...
                    counter!("maluwaf.tunnel.quic.server.access_denied", 
                        "type" => "stream", "client" => session.client_id.clone()).increment(1);
                    
                    let denied_port = if !session.can_access_port(port, &protocol) { port } else { upstream_port };
                    let ack = TunnelMessage::StreamOpenAck {
                        identifier: identifier.clone(),
                        success: false,
                        message: Some(format!("Port {} not allowed for your access level", denied_port)),
                    };
                    write_message(&mut send_stream, &ack).await?;
                    send_stream.finish().map_err(|e| format!("Failed to finish stream: {}", e))?;
                    return Ok(());
                ...
                match tcp_result {
                    ...
                    Err(_) => {
                        ...
                        counter!("maluwaf.tunnel.quic.server.streams.upstream_timeout").increment(1);
            ...
            TunnelMessage::UdpTunnelOpen { identifier, port } => {
                ...
                if !session.can_access_port(port, "udp") {
                    tracing::warn!(
                        ...
                        session.id, session.client_id, session.access_level, port
                    );
                    counter!("maluwaf.tunnel.quic.server.access_denied", 
                        "type" => "udp", "client" => session.client_id.clone()).increment(1);
                    
                    let ack = TunnelMessage::UdpTunnelOpenAck {
                        identifier: identifier.clone(),
                        success: false,
                        message: Some(format!("UDP port {} not allowed for your access level", port)),
                    };
                    write_message(&mut send_stream, &ack).await?;
                    send_stream.finish()?;
                    return Ok(());
    ...
    async fn handle_udp_tunnel(
        ...
        let client_timestamps: Arc<DashMap<SocketAddr, std::time::Instant>> = Arc::new(DashMap::new());
        let datagram_semaphore: Arc<tokio::sync::Semaphore> = Arc::new(tokio::sync::Semaphore::new(MAX_PENDING_DATAGRAMS));
        ...
        let recv_from_local_client = async move {
            ...
            loop {
                match udp_for_local_recv.recv_from(recv_pooled.as_mut_slice()).await {
                    Ok((len, client_addr)) => {
                        ...
                        if client_map_for_local.len() >= MAX_CLIENT_MAPPINGS {
                            tracing::warn!(
                                ...
                                MAX_CLIENT_MAPPINGS, client_addr
                            );
                            counter!("maluwaf.tunnel.quic.server.udp_tunnels.client_map_full").increment(1);
                            continue;
                        }
                        ...
                        let permit = match semaphore_for_local.try_acquire() {
                            Ok(p) => p,
                            Err(_) => {
                                tracing::warn!("UDP backpressure: too many pending datagrams, dropping packet");
                                counter!("maluwaf.tunnel.quic.server.udp_tunnels.backpressure").increment(1);
                                continue;
                            }
                        };
                        ...
                        if let Ok(encoded) = msg.encode() {
                            if encoded.len() <= max_size {
                                match connection_for_quic_send.send_datagram(encoded.into()) { Err(e) => {
                                    tracing::debug!("Failed to send UDP datagram to QUIC: {}", e);
                                } _ => {
                                    counter!("maluwaf.tunnel.quic.server.udp_tunnels.forwarded").increment(1);
                                }}
                            } else {
                                tracing::warn!(
                                ...
                                counter!("maluwaf.tunnel.quic.server.udp_tunnels.oversized").increment(1);
                            }
                        }
                        drop(permit);
                    }
                    ...
                    Ok(data) => {
                        if let Some(msg) = DatagramMessage::decode(&data) {
                            ...
                            if let Some(client_addr) = target_client {
                                if let Err(e) = udp_for_send_to_client.send_to(&msg.data, client_addr).await {
                                    tracing::debug!("Failed to send UDP to client {}: {}", client_addr, e);
                                } else {
                                    counter!("maluwaf.tunnel.quic.server.udp_tunnels.forwarded").increment(1);
                                }
                            } else {
                                tracing::trace!("No client mapping found for sequence {}", msg.sequence);
                            }
                        } else if let Some(msg) = TunnelMessage::decode(&data) {
                            match msg {
                                TunnelMessage::UdpData { identifier: msg_id, data } => {
                                    if msg_id != identifier_for_quic_recv {
                                        continue;
                                    }
                                    
                                    if let Some(client_addr) = client_map_for_quic.iter().next().map(|e| *e.value()) {
                                        if let Err(e) = udp_for_send_to_client.send_to(&data, client_addr).await {
                                            tracing::debug!("Failed to send UDP to client {}: {}", client_addr, e);
                                        } else {
                                            counter!("maluwaf.tunnel.quic.server.udp_tunnels.forwarded").increment(1);
                                        }
    ...
    fn authenticate_client(client_id: &str, auth_token: &str, config: &TunnelQuicConfig) -> Option<AuthResult> {
        if let Some(client_config) = config.server.clients.get(client_id) {
            if client_config.enabled && !client_config.auth_token.is_empty() 
                && auth_token.as_bytes().ct_eq(client_config.auth_token.as_bytes()).into() {
    ...
    fn authenticate_peer(peer_id: &str, auth_token: &str, config: &TunnelQuicConfig) -> Option<TunnelQuicPeerConfig> {
