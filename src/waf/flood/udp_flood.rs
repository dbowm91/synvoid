...
impl UdpFloodProtector {
    ...
    pub fn check_packet_with_port(&self, ip: IpAddr, port: Option<u16>) -> FloodDecision {
        ...
        if global > self.global_rate as u64 {
            metrics::counter!("rustwaf.udp_flood.global_limited").increment(1);
        ...
        if ip_count > self.per_ip_rate {
            metrics::counter!("rustwaf.udp_flood.ip_limited").increment(1);
            return FloodDecision::RateLimited;
        }
        ...
        if let Some(p) = port {
            ...
            if port_count > port_threshold as u32 {
                metrics::counter!("rustwaf.udp_flood.port_limited").increment(1);
                return FloodDecision::RateLimited;
            }
    ...
    fn check_protocol_rate(&self, ip: IpAddr, port: u16, slot: usize) -> FloodDecision {
        match port {
            53 => {
                let count = self.dns_counters[slot].fetch_add(1, Ordering::Relaxed) + 1;
                if count > self.protocol_limits.dns_limit {
                    metrics::counter!("rustwaf.udp_flood.dns_limited").increment(1);
                    return FloodDecision::RateLimited;
                }
            }
            123 => {
                ...
                if count > self.protocol_limits.ntp_limit {
                    metrics::counter!("rustwaf.udp_flood.ntp_limited").increment(1);
                    return FloodDecision::RateLimited;
                }
            }
            161 | 162 => {
                ...
                if count > self.protocol_limits.snmp_limit {
                    metrics::counter!("rustwaf.udp_flood.snmp_limited").increment(1);
                    return FloodDecision::RateLimited;
                }
            }
            1900 => {
                ...
                if count > self.protocol_limits.ssdp_limit {
                    metrics::counter!("rustwaf.udp_flood.ssdp_limited").increment(1);
                    return FloodDecision::RateLimited;
                }
            }
            _ => {}
    ...
    pub fn check_amplification(
        ...
        if ratio > self.protocol_limits.amplification_threshold {
            ...
            if tracker.len() >= MAX_AMPLIFICATION_ENTRIES {
                metrics::counter!("rustwaf.udp_flood.amplification_tracker_full").increment(1);
                return false;
            }
            ...
            if total_ratio > self.protocol_limits.amplification_threshold {
                ...
                metrics::counter!("rustwaf.udp_flood.amplification_detected").increment(1);
                return true;
            }
        }
