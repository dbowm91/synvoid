    ...
    pub async fn start(&self) {
        ...
        for instance in listeners {
            ...
            tokio::spawn(async move {
                Self::listen_loop(config, shutdown_rx, detector, filter, rate_limiter, flood_protector, sock_opts, semaphore, buffer_size).await;
            });
        }

        tracing::info!("TCP listener pool started with {} listeners (max_conns: {}, buffer: {}KB)", 
            listener_count, self.config.max_connections, buffer_size / 1024);
    }
    ...
    async fn listen_loop(
        ...
        let listener = {
            ...
            match TcpListener::from_std(std_listener) {
                Ok(l) => l,
                Err(e) => {
                    tracing::error!("Failed to convert to tokio listener for {}: {}", bind_addr, e);
                    return;
                }
            }
        };
