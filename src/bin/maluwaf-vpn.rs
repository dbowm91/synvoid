...
use maluwaf::vpn_client::{VpnClient, VpnClientConfig, ReconnectConfig};
...
enum Commands {
    ...
    Connect {
        ...
        #[arg(long, default_value = "10", help = "Maximum reconnect attempts (0 = unlimited)")]
        max_retries: u32,
    },
    ...
    Serve {
        #[arg(long, default_value = "127.0.0.1:8080", help = "Dashboard listen address")]
        bind: String,
...
fn parse_port_mapping(s: &str) -> Result<(u16, u16), String> {
    ...
    if parts.len() != 2 {
        return Err(format!("Invalid port mapping format: {}. Expected local:remote", s));
    }
...
async fn run_connect(
    ...
    for mapping in tcp_mappings {
        let (local, remote) = parse_port_mapping(&mapping)
            .map_err(|e| format!("Port mapping error: {}", e))?;
...
fn generate_sample_config(output: &PathBuf) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
...
fn load_config_from_file(path: &PathBuf) -> Result<VpnClientConfig, Box<dyn std::error::Error + Send + Sync>> {
...
async fn main() {
    ...
    let result: Result<(), Box<dyn std::error::Error + Send + Sync>> = match args.command {
        Commands::Connect {
            server, port, client_id, token, tcp, udp, bind, server_name, verify, reconnect, max_retries
        } => {
            run_connect(
                server, port, client_id, token, tcp, udp, bind, server_name, verify, reconnect, max_retries
            ).await
        }
        Commands::ConnectConfig { config } => {
            match load_config_from_file(&config) {
                Ok(vpn_config) => {
                    let vpn_client = VpnClient::new(vpn_config)
                        .expect("Failed to create VPN client");
                    vpn_client.run_with_auto_reconnect().await
                }
                Err(e) => {
