...
impl MinifierClient {
    ...
    pub fn request_minify(
        ...
        let request = Message::CpuTaskRequest {
            request_id,
            task_kind: crate::process::CpuTaskKind::Minify,
            priority: crate::process::CpuTaskPriority::Normal,
            policy: crate::process::CpuTaskPolicy::SkipTransform,
            deadline_unix_ms,
            payload_size_limit: 1024 * 1024,
            output_size_limit: 16 * 1024 * 1024,
            file_payload_path: None,
            payload: crate::process::CpuTaskPayload::Minify {
    ...
    pub fn get_compressed(
        ...
        let request = Message::CpuTaskRequest {
            request_id,
            task_kind: crate::process::CpuTaskKind::GetCompressed,
            priority: crate::process::CpuTaskPriority::Normal,
            policy: crate::process::CpuTaskPolicy::SkipTransform,
            deadline_unix_ms,
            payload_size_limit: 1024 * 1024,
            output_size_limit: 16 * 1024 * 1024,
            file_payload_path: None,
            payload: crate::process::CpuTaskPayload::GetCompressed {
                site_id: site_id.to_string(),
                path: path.to_string(),
...
impl AsyncCpuTaskConnection {
    ...
    fn cancel(&self, request_id: u64, task_kind: crate::process::CpuTaskKind) -> bool {
        if self.is_closed() {
            return false;
        }
    ...
    W: AsyncWrite + Unpin,
{
    if let Some(signer) = signer {
        let data = crate::process::SignedIpcMessage::serialize_signed(message, signer)?;
        writer.write_all(&data).await?;
        writer.flush().await?;
        Ok(())
    } else {
        crate::process::ipc_framing::write_message(writer, message).await
    }
}
    ...
    R: AsyncRead + Unpin,
{
    if let Some(signer) = signer {
        ...
        if total_len > crate::process::ipc_signed::MAX_IPC_MESSAGE_SIZE {
            crate::process::ipc_signed::increment_oversized_rejected();
        ...
        crate::process::SignedIpcMessage::deserialize_signed(&raw, signer).map(Some)
    } else {
        crate::process::ipc_framing::read_message(reader, buffer).await
    }
}
...
impl AsyncCpuTaskConnectionPool {
    ...
    async fn acquire_for_task_kind(
        ...
        let max_in_flight_for_task = match task_kind {
            ...
            crate::process::CpuTaskKind::YaraScan => 1,
...
impl AsyncMinifierClient {
    ...
    pub async fn request_minify(
        ...
        let connection = self
            .pool
            .acquire_for_task_kind(crate::process::CpuTaskKind::Minify, self.timeout_ms)
            .await?;
    ...
    pub async fn get_compressed(
        ...
        let connection = self
            .pool
            .acquire_for_task_kind(crate::process::CpuTaskKind::GetCompressed, self.timeout_ms)
            .await?;
...
fn map_cpu_task_error_for_minifier(
    code: crate::process::CpuTaskErrorCode,
    message: String,
) -> MinifierClientError {
    match code {
...
impl PoisonImageClient {
    ...
    pub async fn poison_image(
        ...
        let connection = self
            .pool
            .acquire_for_task_kind(crate::process::CpuTaskKind::PoisonImage, self.timeout_ms)
            .await
        ...
        let request = crate::process::Message::CpuTaskRequest {
            request_id,
            task_kind: crate::process::CpuTaskKind::PoisonImage,
            priority: crate::process::CpuTaskPriority::Normal,
            policy: crate::process::CpuTaskPolicy::DegradeToInlineSmallOnly,
            deadline_unix_ms,
            payload_size_limit: 64 * 1024 * 1024,
            output_size_limit: 64 * 1024 * 1024,
            file_payload_path,
            payload: crate::process::CpuTaskPayload::PoisonImage {
                site_id: site_id.to_string(),
                body: payload_body,
                last_modified,
                level,
        ...
        match response {
            crate::process::Message::PoisonImageResponse {
                request_id: resp_id,
                poisoned_body,
            } => {
                if resp_id == request_id {
            ...
            crate::process::Message::PoisonImageError {
                request_id: resp_id,
                error,
            } => {
                if resp_id == request_id {
...
fn map_cpu_task_error_for_yara(
    ...
    match code {
        ...
        crate::process::CpuTaskErrorCode::InternalError => YaraScanClientError::ScanFailed(message),
    }
}
...
impl YaraScanClient {
    ...
    pub async fn scan_bytes(
        ...
        let connection = self
            .pool
            .acquire_for_task_kind(crate::process::CpuTaskKind::YaraScan, self.timeout_ms)
            .await
        ...
        let request = crate::process::Message::CpuTaskRequest {
            request_id,
            task_kind: crate::process::CpuTaskKind::YaraScan,
            priority: crate::process::CpuTaskPriority::High,
            policy: crate::process::CpuTaskPolicy::FailClosed,
            deadline_unix_ms,
            payload_size_limit: 64 * 1024 * 1024,
            output_size_limit: 4 * 1024 * 1024,
            file_payload_path,
            payload: crate::process::CpuTaskPayload::YaraScan {
                site_id: site_id.to_string(),
                body: payload_body,
                excluded_categories,
            },
        ...
        match response {
            crate::process::Message::CpuTaskResponse {
                request_id: resp_id,
                task_kind: crate::process::CpuTaskKind::YaraScan,
                result: crate::process::CpuTaskResult::YaraScan { matches },
            } => {
                if resp_id == request_id {
                    return Ok(matches);
                }
...
mod tests {
    ...
    async fn async_minifier_client_demuxes_out_of_order_responses() {
        ...
        let server_socket_path = crate::process::ipc_transport::IpcEndpoint::new(endpoint_name)
            .socket_path()
            .to_path_buf();
        ...
        let server = tokio::spawn(async move {
            ...
            let (first_id, first_path) = match first {
                Message::CpuTaskRequest {
                    request_id,
                    task_kind: crate::process::CpuTaskKind::Minify,
                    payload: crate::process::CpuTaskPayload::Minify { path, .. },
                    ..
                } => (request_id, path),
