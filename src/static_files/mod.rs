...
impl StaticFileHandler {
    ...
    async fn serve_file(
        ...
        if let Some(modified_since) = if_modified_since {
            if let Ok(since) = httpdate::parse_http_date(modified_since) {
                if mtime <= since {
                    return Ok(StaticResponse {
        ...
        if let Ok(mtime_str) = httpdate::fmt_http_date(mtime) {
...
mod httpdate {
    ...
    pub fn fmt_http_date(time: SystemTime) -> std::io::Result<String> {
    ...
    pub fn parse_http_date(s: &str) -> Result<SystemTime, std::io::Error> {
