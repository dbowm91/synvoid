...
use crate::mesh::dht::routing::contact::{PeerContact, GeoInfo};
...
impl DhtRoutingManager {
    pub fn new(config: Arc<MeshConfig>) -> Self {
        ...
        let edge_can_respond_privileged = dht_config.map(|d| d.edge_can_respond_privileged).unwrap_or(false);
    ...
    pub async fn init(&self) {
        ...
        *rt = Some(table);
        ...
        tracing::info!("DHT Routing initialized with geo-routing: {}, regional hubs: {}", 
            self.geo_config.enabled, self.hub_config.enabled);
    }
    ...
    pub fn get_seeds_from_config(&self) -> Vec<SeedNode> {
        self.config.seeds.iter().map(|seed| {
    ...
    pub fn get_bootstrap_nodes_from_config(&self) -> Vec<SeedNode> {
        self.config.dht.as_ref()
            .map(|dht| {
                dht.bootstrap_nodes.iter().map(|addr| {
                    SeedNode {
                        node_id: addr.clone(),
                        address: addr.clone(),
                        port: 443,
    ...
    pub async fn find_closest_global(&self, k: usize) -> Vec<PeerContact> {
        ...
        let mut global_peers: Vec<_> = all_peers.into_iter()
    ...
    pub async fn find_closest_to_node_id(&self, target_node_id: &NodeId, k: usize) -> Vec<PeerContact> {
        let rt = self.routing_table.read().await;
        let table = match rt.as_ref() {
            Some(t) => t,
            None => return Vec::new(),
        ...
        for contact in responders {
            if contact.node_id_string != self.routing_manager.local_node_id() {
                self.routing_manager.add_peer(
                    ...
                    contact.address,
                    contact.port,
                    if contact.is_global { MeshNodeRole::Global } else { MeshNodeRole::Edge },
                    contact.latency_ms,
                    contact.is_trusted,
                    contact.geo,
                    contact.pow_nonce,
...
pub trait SeedBootstrapTransport: Send + Sync {
    fn send_to_peer(&self, node_id: &str, message: MeshMessage) -> impl std::future::Future<Output = Result<(), String>> + Send;
}
...
impl DhtQueryExecutor {
    ...
    pub async fn iterative_find_node(
        ...
        let initial_peers = self.routing_manager.find_closest_peers(target_key, REPLICATION_K).await;
        ...
        loop {
            ...
            let limited_peers: Vec<PeerContact> = {
                ...
                peers_to_query.iter().take(ALPHA).map(|p| (*p).clone()).collect()
            };
            ...
            for result in results {
                match result {
                    Some((peer, peers)) if !peers.is_empty() => query.process_response(&peer, peers),
...
pub trait DhtQueryTransport: Send + Sync {
    fn send_and_wait(&self, node_id: &str, message: MeshMessage) -> impl std::future::Future<Output = Result<MeshMessage, String>> + Send;
}
