...
use openraft::storage::{IOFlushed, LogState, RaftLogStorage, RaftSnapshotBuilder, RaftStateMachine};
...
type CommittedLeaderIdOfConfig = openraft::type_config::alias::CommittedLeaderIdOf<GlobalRegistryTypeConfig>;
...
impl GlobalRegistryStateMachine {
    ...
    pub fn get_applied_membership(&self) -> Option<StoredMembershipOf<GlobalRegistryTypeConfig>> {
        self.get_membership_raw().and_then(|m| {
...
impl GlobalRegistryLogStorage {
    ...
    pub fn get_all_entries(&self) -> Vec<(u64, u64, Vec<u8>)> {
        let db_guard = self.db.lock().unwrap();
        let mut stmt = match db_guard.prepare(
            "SELECT id, term, payload FROM log_entries ORDER BY id"
        ) {
            Ok(s) => s,
            Err(_) => return Vec::new(),
        };
    ...
    async fn try_get_log_entries<RB: std::ops::RangeBounds<u64> + Clone + Debug + OptionalSend>(
        ...
        for (index, _term, payload) in entries {
            if index >= start && index < end {
                let log_id = openraft::log_id::LogId::new(
                    committed_leader_id.clone(),
                    index,
                );
                let payload_parsed: EntryPayload<RaftCommand, NodeId, ()> = postcard::from_bytes(&payload)
                    .unwrap_or(EntryPayload::Blank);
...
impl RaftSnapshotBuilder<GlobalRegistryTypeConfig> for GlobalRegistrySnapshotBuilder {
    async fn build_snapshot(&mut self) -> std::io::Result<SnapshotOf<GlobalRegistryTypeConfig>> {
        ...
        let last_membership = self.state_machine.get_applied_membership()
...
impl RaftLogStorage<GlobalRegistryTypeConfig> for GlobalRegistryLogStorage {
    ...
    async fn get_log_state(&mut self) -> std::io::Result<LogState<GlobalRegistryTypeConfig>> {
        ...
        let last_log_id = last_id.map(|id| openraft::log_id::LogId::new(committed_leader_id.clone(), id));
        let last_purged_log_id = first_id.map(|id| openraft::log_id::LogId::new(committed_leader_id, id));
    ...
    async fn save_vote(&mut self, vote: &<GlobalRegistryTypeConfig as RaftTypeConfig>::Vote) -> std::io::Result<()> {
    ...
    async fn append<I>(&mut self, entries: I, _callback: IOFlushed<GlobalRegistryTypeConfig>) -> std::io::Result<()>
    where
    ...
    async fn truncate_after(&mut self, last_log_id: Option<LogIdOf<GlobalRegistryTypeConfig>>) -> std::io::Result<()> {
        match last_log_id {
            Some(id) => {
...
impl RaftStateMachine<GlobalRegistryTypeConfig> for GlobalRegistryStateMachine {
    ...
    async fn applied_state(&mut self) -> std::io::Result<(Option<LogIdOf<GlobalRegistryTypeConfig>>, StoredMembershipOf<GlobalRegistryTypeConfig>)> {
        let committed_leader_id = CommittedLeaderIdOfConfig::new(0, 0);

        let last_applied = self.get_last_applied_log_id().map(|index| {
            openraft::log_id::LogId::new(committed_leader_id.clone(), index)
        });

        let membership = self.get_applied_membership().unwrap_or_else(|| {
    ...
    where
        Strm: futures::Stream<Item = Result<EntryResponder<GlobalRegistryTypeConfig>, std::io::Error>> + Unpin + OptionalSend,
    {
        use futures::StreamExt;

        let mut stream = entries;
        while let Some(result) = stream.next().await {
            let (entry, resp) = result.map_err(|e| std::io::Error::new(std::io::ErrorKind::Other, e))?;
    ...
    async fn install_snapshot(&mut self, meta: &SnapshotMetaOf<GlobalRegistryTypeConfig>, snapshot: Bytes) -> std::io::Result<()> {
    ...
    async fn get_current_snapshot(&mut self) -> std::io::Result<Option<SnapshotOf<GlobalRegistryTypeConfig>>> {
        ...
        let last_membership = self.get_applied_membership()
            .unwrap_or_else(|| openraft::StoredMembership::default());
