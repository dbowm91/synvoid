...
use crate::upstream::pool::{UpstreamPool, Backend};
use super::trait_def::{ProtocolHandler, WafAction, ProtocolError, BoxedHandler};
use super::types::{ProtocolRequest, ProtocolResponse, ProtocolMetrics, ProtocolType};
...
impl GrpcHandler {
    ...
    fn extract_grpc_metadata(headers: &[u8]) -> HashMap<String, String> {
        ...
        while pos + GRPC_FRAME_HEADER_SIZE < headers.len() {
            let length = u32::from_be_bytes([headers[pos + 1], headers[pos + 2], headers[pos + 3], headers[pos + 4]]) as usize;
            
            if pos + GRPC_FRAME_HEADER_SIZE + length > headers.len() {
                break;
            }
            
            let key_data = &headers[GRPC_FRAME_HEADER_SIZE..GRPC_FRAME_HEADER_SIZE + length.min(256)];
...
impl ProtocolHandler for GrpcHandler {
    ...
    fn parse_request(&self, data: &[u8]) -> Result<ProtocolRequest, ProtocolError> {
        if data.len() < GRPC_FRAME_HEADER_SIZE {
            return Err(ProtocolError::Framing("Insufficient data for gRPC frame".to_string()));
        }
        ...
        let (method, path) = if let Some(ref p) = method_path {
            if p.starts_with("/") {
                if let Some((svc, method)) = p.strip_prefix("/").and_then(|s| s.split_once("/")) {
    ...
    fn apply_waf(&self, request: &mut ProtocolRequest, waf: &Arc<WafCore>) -> WafAction {
        ...
        if self.enable_request_validation {
            ...
            if request.body.len() > self.max_message_size {
                tracing::warn!(protocol = "grpc", size = request.body.len(), "gRPC message exceeds max size");
                self.metrics.requests_blocked.fetch_add(1, Ordering::Relaxed);
                return WafAction::Block;
            }
        }
    ...
    fn select_upstream(&self, request: &ProtocolRequest, pool: &UpstreamPool) -> Option<Backend> {
        if let Some(ref upstream_pool) = self.upstream_pool {
            let rt = upstream_pool.clone();
            tokio::runtime::Handle::current().block_on(async {
                rt.select_backend().await
            })
        } else {
            None
        }
