...
impl DrainManager {
    ...
    pub fn all_workers_drained(&self) -> bool {
        let workers = self.workers.read();
        !workers.is_empty() && workers.values().all(|w| w.drain_complete || w.active_connections == 0)
    }

    pub fn total_active_connections(&self) -> u64 {
        self.workers.read().values().map(|w| w.active_connections).sum()
    }
...
impl DrainProtocol {
    ...
    pub async fn drain_worker_with_confirmation(
        ...
        let drain_id = self.send_drain_request(ipc, worker_id, drain_timeout_secs).await?;
