...
| wasmtime 40.0.4 (via yara-x); direct 36.0.15 LTS | **Tracked** | 16 advisory ignores in `deny.toml` (mirrored in `.cargo/audit.toml`) | YARA compilation only for the transitive line; direct 36.0.15 LTS patched for RUSTSEC-2026-0269 (ignore retained solely for transitive 40.0.4, `wasmtime-wasi` unreachable). Reviewed: 2026-09-17; Re-audit: 2026-10-01. See `architecture/dependency_security_baseline_phase25.md` |
