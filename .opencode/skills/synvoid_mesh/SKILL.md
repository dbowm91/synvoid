---
name: synvoid_mesh
description: Mesh networking architecture with DHT-based service discovery, transport lifecycle, and multi-origin routing.
---
...
**Phase 27 protocol extraction**: stable wire/identity vocabulary lives in
`crates/synvoid-mesh-protocol/` (constants, `HybridSignature` envelope,
`ProtocolSigner` Ed25519 verification, replay protection, `ThreatType`/
`ThreatSeverity`/`ThreatIndicator`, `MessageCategory`/`AckStatus` codes, framing).
`synvoid-mesh` depends downward and re-exports compat paths
(`synvoid_mesh::protocol::*` still works). Verification-only consumers use
`synvoid_mesh_protocol::...`, never full `synvoid-mesh`. Runtime pools
(`CryptoVerificationPool`), Raft, DHT, persistence, YARA, transports stay in
`synvoid-mesh`. Cite canonical `crates/synvoid-mesh-protocol` paths for wire types,
never root facades.

**Trust domains (advisory vs. canonical)**: DHT provides advisory, TTL-bound records (discovery, announcements). Raft provides canonical authority state (OrgPublicKey, ThreatIntel, revocation). Policy layer (key_policy, peer_auth decisions) resolves advisory+canonical into actionable trust; services consume policy outputs, not raw advisory records. See `architecture/mesh_trust_domains.md` for classification, invariants, and review checklist.

**YARA validation trust order (corrective pass, Outcome 1)**: full YARA syntax
validation stays in-process in the supervisor composition root
(`src/supervisor/mesh.rs` `BoundaryValidator` → `synvoid_yara::validate_rules_syntax`),
...
- **Mesh attachment orchestration** (Iteration 95, polished Iteration 96, ordering corrected Iteration 97): `src/worker/unified_server/mesh_attachment.rs` — extracted from `startup_plan.rs`. The `attach_mesh()` function is a thin orchestration wrapper delegating to `create_mesh_pipeline()`, `start_required_mesh()`, and `await_optional_mesh_startup()` helpers. Each helper owns one piece of behavior (pipeline creation, required startup, optional startup race loop) and returns typed outputs. Ready-send deduplicated via `send_ready_if_deferred()`. Internal types (`MeshPipelineRuntime`, `RequiredMeshStartInput`/`Output`, `OptionalMeshStartInput`/`Output`) avoid giant function signatures. Iteration 97: optional mesh transitions to `starting` before spawning one-shot tasks; `RequiredMeshStartInput::mesh_status` used consistently.
