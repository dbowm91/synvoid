...
**Trust domains (advisory vs. canonical)**: DHT provides advisory, TTL-bound records (discovery, announcements). Raft provides canonical authority state (OrgPublicKey, ThreatIntel, revocation). Policy layer (key_policy, peer_auth decisions) resolves advisory+canonical into actionable trust; services consume policy outputs, not raw advisory records. See `architecture/mesh_trust_domains.md` for classification, invariants, and review checklist (relevant for any mesh code touching record ingress, peer role validation, or service distribution). **Iteration 8 seam**: `CanonicalTrustReader` (crates/synvoid-mesh/src/mesh/canonical.rs) — narrow read-only snapshot interface for canonical answers (global nodes, org keys, revocation, threat intel, freshness); prefer `dyn CanonicalTrustReader` over direct Raft imports. **Iteration 9**: `peer_auth.rs` exposes `validate_peer_canonical_status` (reader-backed helper for canonical revocation/global-auth subset of peer role checks); first consumer migration behind the seam. **Iteration 10**: test hardening + precise rustdoc for the helper (covers authorized/unauth/revoked/unavailable/stale + composite global-role behavior with `GLOBAL_ORIGIN` exemption made explicit); global-role authorization semantics reviewed (kept `is_global() && !is_origin()` guard intentionally); no production migration or renames. See `plans/mesh_peer_auth_canonical_tests_iteration_10.md` and updated `architecture/mesh_trust_domains.md`. **Iteration 11**: DHT key policy canonical reader migration — `dht/key_policy.rs` has a reader-backed policy helper (`classify_key_authority_with_canonical_reader`) that uses `CanonicalTrustReader` for canonical authority questions while preserving advisory DHT mechanics. Advisory records remain advisory; signed records are not automatically authorized; unknown/unavailable canonical answers are explicit and are not silently treated as trust. Tests cover advisory-only, global-authorized, unauthorized, revoked, unavailable, stale, and unknown canonical cases.
