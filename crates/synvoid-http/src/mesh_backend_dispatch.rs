...
#[cfg(feature = "mesh")]
use synvoid_mesh::mesh::MeshBackendPool;
#[cfg(feature = "mesh")]
...
#[cfg(feature = "mesh")]
use synvoid_proxy::{
    build_headers_to_filter, filter_response_headers_buf, BackendType, RouteTarget,
};
...
pub async fn maybe_handle_mesh_backend(
        ...
        if let Some(backend) = pool.select_backend(upstream_id).await {
            ...
            return Some(match backend.proxy_request(proxy_req).await {
                Ok(resp) => {
                    ...
                    builder = crate::response_helpers::apply_security_headers(
                        builder,
    ...
    Some(Ok(crate::response_builder::build_response_with_alt_svc(
        502,
