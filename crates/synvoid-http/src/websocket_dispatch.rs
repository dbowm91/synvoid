...
use synvoid_proxy::join_upstream_url;
use synvoid_proxy::protocol::trait_def::{ProtocolHandler, WafAction, WafCoreBackend};
use synvoid_proxy::protocol::types::{ProtocolRequest, ProtocolType};
use synvoid_proxy::protocol::websocket::WebSocketHandler;
...
pub async fn handle_websocket_tunnel(
    ...
    let should_close = Arc::new(RunningFlag::new());
    ...
    let client_to_upstream = async {
        while let Some(msg_result) = client_rx.next().await {
            ...
            match action {
                WafAction::Block => {
                    tracing::warn!(client_ip = %client_ip, "WebSocket message blocked by WAF");
                ...
                WafAction::LogOnly => {
                    tracing::debug!(client_ip = %client_ip, "WebSocket message logged by WAF");
