...
use crate::config::{MeshConfig, MeshTlsMode};
...
impl MeshCertManager {
    ...
    pub fn verify_global_node_proof(
        &self,
        proof: &crate::config::GenesisMintingProof,
    ...
    pub fn rotate_certificates(&mut self) -> Result<(), MeshCertError> {
        ...
        let timestamp = synvoid_utils::safe_unix_duration().as_nanos();
...

        {
            ...
            entries.insert(
                ...
                CrlEntry {
                    ...
                    revocation_time: synvoid_utils::safe_unix_duration().as_secs(),
                    reason,
                },
            );
        }
...
mod mesh_pki_tests {
    ...
    fn test_cert_binding_register_and_retrieve() {
        ...
        cfg.tls.mode = Some(crate::config::MeshTlsMode::Strict);
        cfg.tls.ca_path = None;
        let mgr = MeshCertManager::new(&cfg);
