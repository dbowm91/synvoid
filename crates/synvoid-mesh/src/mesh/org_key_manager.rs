...
use crate::dht::record_store::RecordStoreManager;
use crate::organization::{OrgKey, OrgPublicKey, Organization, QuorumSignature};
use crate::raft::{Namespace, RaftAwareClient, RaftCommitNotification};
use base64::Engine;
...
impl OrgKeyManager {
    ...
    pub async fn publish_org_public_key(&self, org: &Organization) -> Result<(), OrgKeyError> {
        ...
        let value = synvoid_utils::serialization::serialize(&pub_key)
            ...
            .await
        {
            Ok(commit_index) => {
                ...
                if let Some(leader_id) = leader_id {
                    let notification = crate::raft::RaftCommitNotification::new(
                        leader_id,
                        commit_index,
                        Namespace::Org,
                        key_id.clone(),
    ...
    pub async fn revoke_global_node(
        ...
        if let Some(store) = self.record_store.read().clone() {
            let dht_key = crate::dht::keys::DhtKey::revoked_global_node(target_node_id);
            let mut record = crate::protocol::DhtRecord {
                key: dht_key.as_str(),
                value,
                timestamp: synvoid_utils::safe_unix_timestamp(),
    ...
    async fn broadcast_raft_commit_notification(
        &self,
        notification: &crate::raft::RaftCommitNotification,
    ) -> Result<(), OrgKeyError> {
        let store = self
            .record_store
            .read()
        ...
        let value = synvoid_utils::serialization::serialize(notification)
            .map_err(|e| OrgKeyError::SerializationError(e.to_string()))?;
    ...
    pub async fn sync_from_dht(&self) -> Result<(), OrgKeyError> {
        ...
        for record in records {
            if let Ok(pub_key) =
                synvoid_utils::serialization::deserialize::<OrgPublicKey>(&record.value)
            {
