...
use crate::dht::{DhtAccessControl, RecordStoreConfig, RecordStoreManager};
use crate::proxy::{MeshProxy, MeshProxyError};
use crate::topology::MeshTopology;
use crate::transport::MeshTransport;
use crate::transports::{MeshTransportManager, QuicMeshTransport};
...
pub fn create_record_store(
    ...
    let store_config = RecordStoreConfig {
        ...
        require_signed_sync_requests: dht_config.require_signed_sync_requests,
        unsigned_sync_compat_until_unix: dht_config.unsigned_sync_compat_until_unix,
    ...
    if let Some(key) = config.global_node_key.as_ref() {
        ...
        let mut s = crate::protocol::MeshMessageSigner::new(key_bytes);
    ...
    let verifier =
        crate::dht::capability_access::CapabilityAccessVerifier::new(|node_id, capability| {
            let key = format!("capability_attestation:{}:{}", node_id, capability);
            crate::get_global_record_store().and_then(|rs| {
                rs.get_record(&key).and_then(|r| {
                    synvoid_utils::serialization::deserialize::<
                        crate::dht::capability_attestation::CapabilityAttestation,
                    >(&r.value)
                    .ok()
                })
            })
...
pub fn create_mesh_backend_from_config(
    ...
    crate::transport::validate_system_time();
    ...
    let cache_settings = config.proxy_cache.as_ref().map(|cc| {
        synvoid_proxy_cache::ProxyCacheSettings::from_config(
            cc.enable,
            cc.path.clone(),
            cc.max_size.clone(),
            cc.inactive,
...
pub async fn initialize_mesh_transports(
    ...
    let verification_pool = Arc::new(crate::crypto_verification::CryptoVerificationPool::default());
    ...
    let stake_manager = config.stake.as_ref().map(|stake_config| {
        ...
        let stake_mgr = crate::dht::StakeManager::new(
            stake_config.clone(),
            config.node_id(),
            is_global,
        );
    ...
    if let Some(erm) = crate::raft::edge_replica::create_edge_replica_manager_with_freshness(
        Some(db_path),
        config.authority_freshness.clone(),
    ) {
