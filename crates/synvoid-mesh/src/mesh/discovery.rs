...
impl MeshDiscovery {
        ...
        self.topology.add_peer(
            crate::protocol::MeshPeerInfo {
                ...
                capabilities: MeshCapabilities {
                    ...
                    preferred_transport: Some(crate::transports::MeshTransportType::Quic),
                },
                is_global: false,
                latency_ms: None,
                upstreams: vec![],
    ...
    async fn connect_to_seed(&self, seed: &MeshSeedNode) -> Result<(), MeshDiscoveryError> {
        ...
        self.topology.add_peer(
            crate::protocol::MeshPeerInfo {
                ...
                role: crate::config::MeshNodeRole::GLOBAL,
                capabilities: MeshCapabilities {
                    can_route: true,
                    can_proxy: true,
                    max_hops: self.config.routing.max_hops,
    ...
    pub async fn handle_seed_list_response(
        &self,
        global_nodes: Vec<crate::protocol::MeshPeerInfo>,
        edge_nodes: Vec<crate::protocol::MeshPeerInfo>,
        version: u64,
    ) {
        ...
        for node in edge_nodes {
            if !self.topology.get_peer(&node.node_id).await.is_some() {
                self.topology.add_peer(
                    node,
                    crate::topology::PeerStatus::Connecting,
                ).await;
            }
        }
    ...
    pub async fn connect_to_peer(&self, address: &str) -> Result<String, MeshDiscoveryError> {
        ...
        let local_upstreams: HashMap<String, crate::protocol::UpstreamInfo> = self
            .topology
            .get_local_upstreams()
            .into_iter()
            ...
            MeshMessage::Hello {
                ...
                if let Some(upstreams) = self.build_hello_upstreams() {
                    ...
                    return Ok(MeshMessage::HelloAck {
                        ...
                        capabilities: crate::protocol::MeshCapabilities::from_config(&self.config, self.config.role),
                        upstreams,
                        auth_token: None,
    ...
    fn build_hello_upstreams(&self) -> Option<HashMap<String, crate::protocol::UpstreamInfo>> {
        if self.topology.is_global() {
            let peers = self.topology.get_all_peers().await;
            let mut upstreams: HashMap<String, crate::protocol::UpstreamInfo> = HashMap::new();

            for peer in peers {
                for upstream_id in peer.upstreams {
                    upstreams.insert(
                        upstream_id.clone(),
                        crate::protocol::UpstreamInfo {
                            upstream_id,
                            upstream_url: None,
                            geo: None,
                            is_local: false,
        ...
        } = msg
        {
            if !signature.is_empty() {
                ...
                if let Some(pubkey) = cert_manager.get_global_node_key(&provider_node_id) {
                    if !crate::cert::verify_ed25519(&sign_data, &signature, &pubkey) {
                        tracing::warn!(
    ...
    fn generate_global_node_auth_sig(&self) -> Option<String> {
        ...
        if let Some(sk) = self.config.signing_key() {
            if sk.len() == 32 {
                ...
                match crate::peer_auth::generate_global_node_auth(&self.topology.node_id(), &key_bytes) {
                    Ok((sig, _ts)) => Some(sig),
                    Err(e) => {
