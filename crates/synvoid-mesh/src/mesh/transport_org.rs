...
use crate::tier_key_encryption::derive_transmission_key;
...
impl MeshTransport {
    pub(crate) async fn handle_org_registration_request(
        ...
        let validated_name = match crate::organization::sanitize_org_name_with_config(
            org_name,
            &org_config.bad_names,
        ) {
            Ok(name) => name,
            Err(e) => {
                tracing::warn!(
        ...
        let pending = crate::organization::OrgPendingRequest::new(
    ...
    pub(crate) async fn auto_approve_organization(
        ...
        let org_key = crate::organization::OrgKey::generate(Some(requesting_node_id.to_string()));

        let mut org = crate::organization::Organization::new(
            Some(org_id.clone()),
        ...
        if org_config.default_tier_on_approve > 0 {
            ...
            let tier_key = crate::organization::TierKey::new(
            ...
            if let Some(ref tier_key) = initial_tier_key {
                if let Some(ref enc) = self.tier_key_encryption {
                    if let Ok(encrypted) = enc.encrypt_tier_key_data(
                        ...
                        let serialized =
                            crate::tier_key_encryption::serialize_encrypted_tier_key(&encrypted);
                        let tier_key_dht =
    ...
    pub(crate) async fn send_org_registration_response(
        ...
        approved: bool,
        reason: String,
        tier_key: Option<&crate::organization::TierKey>,
    ) {
            ...
            if let Some(ref enc) = self.tier_key_encryption {
                if let Some(ref session_mgr) = self.mlkem_session_manager {
                    if let Some(session) = session_mgr.get_by_peer(to_peer) {
                        ...
                        Some(crate::organization::TierKey {
        ...
        let response = crate::protocol::MeshMessage::OrgRegistrationResponse {
                    ...
                    if let Some(ref session_mgr) = self.mlkem_session_manager {
                        if let Some(session) = session_mgr.get_by_peer(from_peer) {
                            let transmission_key =
                                crate::tier_key_encryption::derive_transmission_key(
                                    &session.session_key,
                                );
    ...
    pub(crate) async fn handle_tier_key_announce(
        &self,
        org_id: &str,
        tier_key: &crate::organization::TierKey,
    ) {
        tracing::debug!(
    ...
    pub(crate) async fn handle_unspent_tier_key_announce(
        &self,
        org_id: &str,
        tier_keys: &[crate::organization::TierKey],
    ) {
        tracing::debug!(
    ...
    pub(crate) async fn broadcast_unspent_tier_keys(&self, org_id: &str) -> Result<(), String> {
        ...
        let message = crate::protocol::MeshMessage::UnspentTierKeyAnnounce {
            org_id: org_id.into(),
            tier_keys,
            signature,
            timestamp,
        ...
        let _result = self
            .broadcast_to_random_peers(message, 0.3, Some(crate::config::MeshNodeRole::GLOBAL))
            .await;
    ...
    pub(crate) async fn handle_org_invitation_request(
        ...
        let invitation = crate::organization::OrgInvitation::new(
            request_id.to_string(),
            org_id.to_string(),
        ...
        if let Some(inv) = invitation {
            if let Some(ref pubkey_hex) = inv.invited_node_pubkey {
                if let Ok(pubkey_bytes) = hex::decode(pubkey_hex) {
                    let is_valid = crate::organization::verify_invitation_proof(
                        proof_of_key,
                        invitation_token,
                        org_id,
                        invited_node_id,
