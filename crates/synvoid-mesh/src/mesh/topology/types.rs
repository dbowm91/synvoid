...
impl ShardedPeerStore {
    ...
    pub fn update_peer_status(&self, node_id: &str, status: PeerStatus) {
        ...
        if let Some(peer) = shard.peers.get_mut(node_id) {
            peer.status = status;
            peer.last_seen = synvoid_utils::safe_unix_timestamp();
        }
    }

    pub fn update_peer_latency(&self, node_id: &str, latency_ms: u32) {
        let mut shard = self.shard(node_id).write();
        let avg_latency = shard
            .latency_history
            .get(node_id)
            .map_or(latency_ms, |history| {
                if history.is_empty() {
                    latency_ms
                } else {
...
pub struct PeerState {
    ...
    pub capabilities: crate::protocol::MeshCapabilities,
...
impl PeerState {
    ...
    pub fn idle_secs(&self) -> u64 {
        synvoid_utils::safe_unix_timestamp().saturating_sub(self.last_seen)
    }
}
...
pub struct TopologyFullSync {
    ...
    pub upstreams: HashMap<String, crate::protocol::UpstreamOwner>,
    pub version: u64,
}
...
pub struct TopologyIncrementalDelta {
    ...
    pub added_upstreams: HashMap<String, crate::protocol::UpstreamOwner>,
...
impl PeerScore {
    pub fn calculate_total(&mut self, weights: &crate::config::ConnectionScoreWeights) {
