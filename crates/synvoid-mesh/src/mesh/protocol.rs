...
use crate::crypto_verification::CryptoVerificationPool;
use crate::hybrid_signature::HybridSignature;
use crate::ml_dsa::MeshMlDsaSigner;
    ...
    pub fn verify_hybrid(&self, content: &[u8], hybrid: &HybridSignature) -> bool {
        ...
        if hybrid.has_ml_dsa() {
            if let Some(ref ml_pk_b64) = hybrid.ml_dsa_public_key {
                let verifier = match crate::ml_dsa::MeshMlDsaVerifier::from_base64(ml_pk_b64) {
                    Ok(v) => v,
                    Err(_) => return false,
                };
...
pub enum MeshMessage {
    Hello {
        ...
        member_certificate: Option<crate::organization::MemberCertificate>,
        org_public_key: Option<crate::organization::OrgPublicKey>,
    },
    HelloAck {
        version: u8,
        node_id: ArcStr,
    ...
    TierKeyAnnounce {
        org_id: ArcStr,
        key: crate::organization::TierKey,
        signature: Vec<u8>,
    },
    TierKeyRevoke {
        org_id: ArcStr,
    ...
    TierKeyQueryResponse {
        request_id: ArcStr,
        keys: Vec<crate::organization::TierKey>,
        signature: Vec<u8>,
    },
    UnspentTierKeyAnnounce {
        org_id: ArcStr,
        tier_keys: Vec<crate::organization::TierKey>,
        signature: Vec<u8>,
        timestamp: u64,
    },
    ...
    OrgRegistrationResponse {
        ...
        approved: bool,
        reason: ArcStr,
        initial_tier_key: Option<crate::organization::TierKey>,
        signature: Vec<u8>,
        timestamp: u64,
    },
    ...
    OrgInvitationResponse {
        ...
        org_id: ArcStr,
        accepted: bool,
        org_key: Option<crate::organization::TierKey>,
        reason: ArcStr,
        signature: Vec<u8>,
        timestamp: u64,
    },
    GlobalNodeAnnounce {
        ...
        cert_chain: Option<crate::cert::CertChain>,
    },
    OrgMemberAnnounce {
        org_id: ArcStr,
    ...
    ReplicaSyncResponse {
        ...
        entries: Vec<crate::raft::RaftCommitNotification>,
    },
    MeshLoadUpdate {
        request_id: ArcStr,
        record: DhtRecord,
    ...
    PeerBlocked {
        ...
        reason: ArcStr,
        blocked_by: ArcStr,
        evidence_receipt: Option<crate::dht::AuditReceipt>,
    },
    BandwidthReport {
        upstream_id: ArcStr,
        bytes_sent: u64,
    ...
    DnsRegistrationRequest {
        request_id: ArcStr,
        registration: synvoid_dns::messages::DnsRegistrationWithVerificationRequest,
        timestamp: u64,
    },
    #[cfg(feature = "dns")]
    DnsRegistrationResponse {
        request_id: ArcStr,
        response: synvoid_dns::messages::DnsRegistrationWithVerificationResponse,
        timestamp: u64,
    },
    #[cfg(feature = "dns")]
    DnsVerificationUpdate {
        request_id: ArcStr,
        update: synvoid_dns::messages::DomainVerificationStatusUpdate,
        timestamp: u64,
    },
    ...
    ConsistentReadRequest {
        request_id: ArcStr,
        namespace: crate::raft::state_machine::Namespace,
        key: ArcStr,
        requesting_node_id: ArcStr,
        timestamp: u64,
    },
...
impl From<&synvoid_config::site::ProxyCacheConfig> for ProxyCachePreferences {
    fn from(config: &synvoid_config::site::ProxyCacheConfig) -> Self {
        Self {
...
impl MeshCapabilities {
    pub fn from_config(config: &crate::config::MeshConfig, role: MeshNodeRole) -> Self {
        use crate::transports::MeshTransportType;
...
impl DhtRecord {
    ...
    pub fn verify_for_ingress(
        &self,
        ctx: &crate::dht::signed::DhtRecordIngressContext,
        access_control: &crate::dht::DhtAccessControl,
    ) -> Result<(), DhtRecordVerificationError> {
        if !self.verify_content_hash() {
            return Err(DhtRecordVerificationError::InvalidContentHash);
        }

        if !crate::dht::signed::validate_record_timestamp(self.timestamp) {
        ...
        if ctx.source_classification() != crate::dht::signed::SourceClassification::LocalNode {
            ...
            if !crate::dht::signed::verify_dht_record_signature(self) {
...
impl From<&crate::dht::quorum::QuorumSignature> for QuorumSignatureProto {
    fn from(s: &crate::dht::quorum::QuorumSignature) -> Self {
        Self {
...
impl ServerlessPermissionClaim {
    ...
    pub fn sign(&mut self, key: &[u8]) {
        ...
        self.signature = crate::cert::sign_ed25519(&data, key).unwrap_or_default();
    }
    ...
    pub fn verify_signature(&self, key: &[u8]) -> bool {
        ...
        let data = self.get_signable_data();
        let Some(public_key) = crate::cert::get_ed25519_public_key(key) else {
            return false;
        };
        crate::cert::verify_ed25519(&data, &self.signature, &public_key)
    }
