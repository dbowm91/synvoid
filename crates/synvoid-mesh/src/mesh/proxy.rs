...
use crate::organization::OrganizationManager;
use crate::protocol::{ProviderInfo, UpstreamProtocol, WafPolicy};
use crate::stubs::metrics::bandwidth::get_global_bandwidth_tracker_or_log;
use crate::topology::MeshTopology;
use crate::transport::MeshTransport;
use synvoid_proxy::headers::is_hop_by_hop_header_name;
use synvoid_proxy_cache::key::CacheKeyBuilder;
use synvoid_proxy_cache::ProxyCache;
use synvoid_proxy_cache::ProxyCacheSettings;
...
pub struct MeshProxy {
    ...
    transport_manager: Arc<RwLock<Option<Arc<crate::transports::manager::MeshTransportManager>>>>,
...
impl MeshProxy {
    ...
    pub fn set_proxy_cache_preferences(
        &self,
        preferences: &crate::protocol::ProxyCachePreferences,
    ) {
        ...
        match proxy_cache.as_mut() {
            Some(cache) => {
                cache.update_settings(|settings| {
                    ...
                    settings.valid_status =
                        preferences.valid_status.iter().map(|&v| v as u16).collect();
    ...
    pub fn set_transport_manager(
        &self,
        manager: Arc<crate::transports::manager::MeshTransportManager>,
    ) {
        let mut m = self.transport_manager.write();
        *m = Some(manager);
    }

    pub async fn register_organization(&self, org: crate::organization::Organization) {
    ...
    pub async fn issue_tier_key(
        ...
        valid_from: u64,
        valid_until: u64,
    ) -> Option<crate::organization::TierKey> {
        let mut mgr = self.org_manager.write().await;
        mgr.issue_tier_key(
            org_id,
            tier,
    ...
    pub async fn validate_tier_claim(&self, claim: &crate::organization::TierClaim) -> bool {
                    ...
                    Ok(result) => {
                        ...
                        if tier_filtered.is_empty() && !providers.is_empty() {
                            ...
                            let alternatives: Vec<_> = providers
                                .iter()
                                .map(|p| crate::protocol::AlternativeProvider {
        ...
        B::Error: std::fmt::Debug + Send,
    {
        ...
        loop {
            ...
            let provider_info = if let Some(ref cached) = cached {
                if let Some(peer) = self.topology.get_peer(&cached.provider_node_id).await {
                    if peer.is_healthy() {
                        Some(crate::protocol::ProviderInfo {
                ...
                Err(e) => {
                    ...
                    if let Some(ref tm) = tm {
                        tm.report_reachability(
                            upstream_id,
                            &provider_node_id,
                            crate::dht::ReachabilityStatus::Failed,
                            0,
                            0.0,
                            failure_count,
                        );
                    ...
                    if !Self::should_bypass_cache(&request_headers) {
                        ...
                        if let Some(cached_entry) = cache.get(&cache_key).await {
                            ...
                            return Ok(builder
                                ...
                                .unwrap_or_else(|_| crate::stubs::http::fallback_error_boxed()));
                        }
                            ...
                            let body_bytes = match resp.into_body().collect().await {
                                ...
                                Err(e) => {
                                    ...
                                    return Ok(HttpResponse::builder()
                                        ...
                                        .unwrap_or_else(|_| {
                                            crate::stubs::http::fallback_error_boxed()
                                        }));
                                }
                            };
    ...
    async fn transform_response(
...

        {
            ...
            if let Some(ref record_store) = *rs {
                let dht_key = crate::dht::keys::DhtKey::transformed_content(
                    upstream_id,
                    &content_hash,
                    &transform_flags,
                );
        ...
        if let Some(ref config) = minification {
            if config.enabled.unwrap_or(false) {
                let settings = crate::stubs::http::response_transform::MinificationSettings {
                    enabled: true,
                ...
                transformed = crate::stubs::http::response_transform::apply_minification(
                    transformed,
                    Some(&content_type),
                    &settings,
                );
        ...
        if let Some(ref comp_config) = compression {
            if comp_config.enabled.unwrap_or(false) {
                ...
                let settings = crate::stubs::http::response_transform::CompressionSettings {
                    enabled: true,
                ...
                let (compressed_body, encoding) =
                    crate::stubs::http::response_transform::apply_compression(
                        transformed.clone(),
                        accept_encoding,
                        &settings,
                    );
    ...
    pub async fn announce_upstream(
        ...
        match action {
            crate::protocol::AnnounceAction::Add => {
                self.topology
