...
use crate::transport::{MeshPeerConnection, MeshTransport, MeshTransportError};
use std::time::{Duration, Instant};

use quinn::SendStream;
use rand::Rng;

use crate::protocol::{MeshMessage, ProviderInfo, RouteQueryResult};
...
impl MeshTransport {
    ...
    pub(crate) async fn handle_route_response(
        ...
        timestamp: u64,
        signature: Vec<u8>,
        upstream_url: Option<crate::protocol::ArcStr>,
        waf_policy: Option<crate::protocol::WafPolicy>,
        priority_tier: u32,
        tier_claim: Option<crate::organization::TierClaim>,
        org_id: Option<crate::protocol::ArcStr>,
        mesh_name: Option<crate::protocol::ArcStr>,
    ) {
        tracing::debug!(
        ...
        if !signature.is_empty() {
            ...
            if let Some(pubkey) = cert_manager.get_global_node_key(provider_node_id) {
                if let Some(ref signer) = self.mesh_signer {
                    if !signer.verify(sign_data.as_bytes(), &signature, &pubkey) {
                        tracing::warn!(
