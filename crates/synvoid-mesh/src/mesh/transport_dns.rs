...
#[cfg(feature = "dns")]
use synvoid_dns::server::Zone as DnsZone;
...
impl MeshTransport {
    pub(crate) async fn handle_anycast_registration(
        ...
        _from_peer: &str,
        _request_id: &str,
        registration: synvoid_dns::messages::DnsAnycastNodeRegistration,
    ) {
        tracing::debug!(
    ...
    pub(crate) async fn handle_anycast_health_update(
        ...
        let update = synvoid_dns::messages::DnsAnycastHealthUpdate {
    ...
    pub(crate) async fn handle_zone_sync_request(
            ...
            if is_origin {
                ...
                if let Some(zone) = zone_opt {
                    ...
                    if client_serial == current_serial {
                        ...
                        let records: Vec<synvoid_dns::anycast_sync::SerializedRecord> = zone
                            .records
                            .iter()
                            .flat_map(|((name, rt), records)| {
                                records
                                    .iter()
                                    .map(|r| synvoid_dns::anycast_sync::SerializedRecord {
                                        name: name.clone(),
                        ...
                        let json =
                            serde_json::to_string(&synvoid_dns::anycast_sync::SerializedZoneData {
                        ...
                        if let Some(old_records) = old_records {
                                ...
                                match (new_recs, old_recs) {
                                    (Some(new), Some(old)) => {
                                        ...
                                        if changed {
                                            ...
                                            changes.push(synvoid_dns::anycast_sync::ZoneChange {
                            ...
                            let json = serde_json::to_string(
                                &synvoid_dns::anycast_sync::SerializedIxfrData {
                                    origin: zone.origin.clone(),
                                    serial: zone.serial,
                            ...
                            let json = serde_json::to_string(
                                &synvoid_dns::anycast_sync::SerializedZoneData {
                                    origin: zone.origin.clone(),
                                    serial: zone.serial,
                                    records,
                                    history: vec![],
        ...
        let response = crate::protocol::MeshMessage::ZoneSyncResponse {
        ...
        if let Some(records_arr) = data.get("records").and_then(|r| r.as_array()) {
            for rec in records_arr {
                ...
                records
                    .entry(key)
                    .or_default()
                    .push(synvoid_dns::DnsZoneRecord {
                        name,
                        record_type,
                        value,
                        ttl,
    ...
    pub(crate) async fn handle_node_shutdown(
        ...
        if graceful && time_until_shutdown > 0 {
            if let Some(dns_registry) = &self.dns_registry {
                let shutdown_msg = synvoid_dns::messages::DnsNodeShutdown {
                    node_id: node_id.to_string(),
                    role: if role.is_edge() {
                        synvoid_dns::messages::DnsNodeRole::Edge
                    } else {
                        synvoid_dns::messages::DnsNodeRole::Origin
                    },
    ...
    pub(crate) async fn handle_dns_domain_register_request(
        ...
        if verified {
            let registration = synvoid_dns::messages::DnsRegistration {
                ...
                role: synvoid_dns::messages::DnsNodeRole::Origin,
                edge_node_id: None,
                edge_node_geo: None,
    ...
    pub(crate) async fn verify_signed_challenge(
        ...
        if synvoid_integrity::signing::verify_ed25519_raw(
            &public_key_bytes,
            &challenge_content,
            &signature_bytes,
        ) {
