...
use crate::raft::state_machine::{ClientProposalPayload, CommandKind, Namespace, RaftCommand};
...
impl RaftAwareClient {
    ...
    pub async fn raft_write(
        ...
        if self.config.role.is_global() {
            ...
            match tokio::time::timeout(timeout, self.raft_write_via_global(namespace, key, value))
                .await
            {
                Ok(res) => res,
                Err(_) => {
    ...
    async fn send_client_proposal(
        ...
        let request_id = uuid::Uuid::new_v4().to_string();
        let command_bytes = synvoid_utils::serialization::serialize(&command)
            .map_err(|e| RaftAwareClientError::RaftWriteFailed(e.to_string()))?;

        let raft_payload = crate::protocol::RaftPayload {
            msg_type: crate::protocol::RaftMsgType::ClientProposal,
            request_id: Some(request_id),
            data: command_bytes,
        };

        let raft_msg = crate::protocol::MeshMessage::Raft {
        ...
        let response: crate::protocol::MeshMessage =
            crate::protocol::MeshMessage::decode(&response_data).ok_or_else(|| {
                RaftAwareClientError::InvalidResponse("Failed to decode response".to_string())
            })?;

        match response {
            crate::protocol::MeshMessage::ConsistentReadResponse { value: Some(v), .. } => {
            ...
            crate::protocol::MeshMessage::NotLeader {
