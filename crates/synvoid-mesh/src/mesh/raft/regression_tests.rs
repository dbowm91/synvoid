...
mod signed_record_tests {
    use crate::dht::signed::{RecordSigner, SignedDhtRecord, SignedRecordType};
    use base64::Engine;
    ...
    fn test_signed_record_empty_signature_rejected() {
        ...
        let signer = RecordSigner::new(Some(crate::protocol::MeshMessageSigner::new([0x42u8; 32])));
    ...
    fn test_signed_record_invalid_signature_rejected() {
        ...
        let signer = RecordSigner::new(Some(crate::protocol::MeshMessageSigner::new([0x99u8; 32])));
        assert!(!signer.verify(&record));
    }
    ...
    fn test_signed_record_tampered_value_rejected() {
        ...
        let signer = RecordSigner::new(Some(crate::protocol::MeshMessageSigner::new(signing_key)));
...
mod dht_adversarial_tests {
    use crate::dht::signed::{validate_message_timestamp, SignedDhtRecord, SignedRecordType};
    use synvoid_utils::safe_unix_timestamp;
...
mod raft_command_tests {
    use crate::raft::state_machine::RaftCommand;

    #[test]
    fn test_raft_command_set_serialization() {
        let cmd = RaftCommand::Set {
            namespace: crate::raft::state_machine::Namespace::Org,
        ...
        let serialized = synvoid_utils::serialization::serialize(&cmd).unwrap();
        let deserialized: RaftCommand =
            synvoid_utils::serialization::deserialize(&serialized).unwrap();
        ...
        match deserialized {
            RaftCommand::Set {
                ...
                assert_eq!(namespace, crate::raft::state_machine::Namespace::Org);
    ...
    fn test_raft_command_delete_serialization() {
        let cmd = RaftCommand::Delete {
            namespace: crate::raft::state_machine::Namespace::Intel,
        ...
        match deserialized {
            RaftCommand::Delete {
                ...
                assert_eq!(namespace, crate::raft::state_machine::Namespace::Intel);
    ...
    fn test_raft_command_delete_display() {
        let cmd = RaftCommand::Delete {
            namespace: crate::raft::state_machine::Namespace::Revocation,
...
mod edge_replica_tests {
    ...
    use crate::raft::state_machine::{Namespace, OrgPublicKey, ThreatIntel};
...
mod mesh_message_raft_tests {
    use crate::protocol::{ArcStr, MeshMessage, RaftMsgType, RaftPayload};
    ...
    fn test_installsnapshot_header_encode_decode() {
        ...
        let decoded: crate::protocol::SnapshotHeader =
    ...
    fn test_installsnapshot_chunk_encode_decode() {
        ...
        let decoded: crate::protocol::SnapshotChunk =
            postcard::from_bytes(&bytes).expect("Failed to deserialize");
...
mod snapshot_install_tests {
    use crate::transport::InProgressSnapshot;
...
mod dht_snapshot_signable_tests {
    use crate::dht::signed::{get_snapshot_signable_content, get_sync_signable_content};
...
mod streaming_snapshot_tests {
    use crate::raft::state_machine::{GlobalRegistryStateMachine, Namespace, RaftSnapshotData};
...
mod regression_tests {
    ...
    fn test_regression_raft_log_reload_uses_term_0_instead_of_persisted_terms() {
        use crate::raft::state_machine::GlobalRegistryLogStorage;
    ...
    fn test_regression_snapshot_request_signature_mismatch_between_sign_and_verify() {
        use crate::dht::signed::get_anti_entropy_request_signable_content;
