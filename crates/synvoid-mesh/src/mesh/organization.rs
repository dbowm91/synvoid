        ...
        for sig in &self.quorum_signatures {
            ...
            if let Some(expected_pubkey_b64) = authorized_global_keys.get(&sig.signer_node_id) {
                if let Some(verifier) =
                    synvoid_integrity::protocol::Ed25519Verifier::from_base64(expected_pubkey_b64)
                {
...
impl OrgKey {
    ...
    pub fn sign(&self, data: &str) -> Vec<u8> {
        crate::cert::sign_ed25519(data, &self.private_key).unwrap_or_default()
    }

    pub fn verify(&self, data: &str, signature: &[u8]) -> bool {
        crate::cert::verify_ed25519(data, signature, &self.public_key)
    }
...
impl MemberCertificate {
    ...
    pub fn verify(&self, org_private_key: &[u8]) -> bool {
        ...
        let Some(org_public_key) = crate::cert::get_ed25519_public_key(org_private_key) else {
            return false;
        };
        crate::cert::verify_ed25519(&self.get_signable_data(), &self.signature, &org_public_key)
    }
...
impl Organization {
    ...
    pub fn update_name(
        &mut self,
        new_name: String,
    ) -> Result<String, crate::organization::NameValidationError> {
        let validated = crate::organization::sanitize_org_name(&new_name)?;
...
impl TierKey {
    ...
    pub fn revoke(&mut self) {
        self.revoked = true;
        self.revoked_at = Some(synvoid_utils::safe_unix_timestamp());
    }
    ...
    pub fn accept_invitation(&mut self, node_id: &str) -> Option<OrgInvitation> {
        if let Some(inv) = self.invitations.get_mut(node_id) {
            if inv.status == OrgInvitationStatus::Pending && !inv.is_expired() {
                ...
                inv.accepted_at = Some(synvoid_utils::safe_unix_timestamp());
