...
impl VerificationTaskManager {
    ...
    pub fn record_verification_result(
        ...
        let mut task =
            match serde_json::from_slice::<crate::dht::VerificationTask>(&task_record.value) {
                Ok(t) => t,
                Err(e) => {
        ...
        if task.status != crate::dht::VerificationStatus::InProgress {
        ...
        task.verification_results
            .push(crate::dht::VerificationResult {
        ...
        if total_responses >= total_expected {
            task.status = crate::dht::VerificationStatus::Completed;
