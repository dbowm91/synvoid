        ...
        match payload {
            proto::mesh_message::Payload::Hello(h) => {
                ...
                Ok(MeshMessage::Hello {
                    ...
                    org_public_key: h.org_public_key.map(|k| crate::organization::OrgPublicKey {
                        org_id: k.org_id,
                        key_id: k.key_id,
                        ...
                        quorum_signatures: k
                            .quorum_signatures
                            .into_iter()
                            .map(|s| crate::organization::QuorumSignature {
            ...
            proto::mesh_message::Payload::TierKeyAnnounce(t) => Ok(MeshMessage::TierKeyAnnounce {
                ...
                key: t
                    .key
                    .map(|k| crate::organization::TierKey {
                        key_id: k.key_id,
                        tier: k.tier,
                        key: k.key,
                    ...
                    .unwrap_or_else(|| crate::organization::TierKey {
                        key_id: String::new(),
                        tier: 0,
                        key: Vec::new(),
                        valid_from: 0,
            ...
            proto::mesh_message::Payload::OrgRegistrationResponse(r) => {
                Ok(MeshMessage::OrgRegistrationResponse {
                    ...
                    approved: r.approved,
                    reason: r.reason.into(),
                    initial_tier_key: r.initial_tier_key.map(|k| crate::organization::TierKey {
                        key_id: k.key_id,
                        tier: k.tier,
                        key: k.key,
                        valid_from: k.valid_from,
            ...
            proto::mesh_message::Payload::OrgInvitationResponse(r) => {
                Ok(MeshMessage::OrgInvitationResponse {
                    ...
                    org_id: r.org_id.into(),
                    accepted: r.accepted,
                    org_key: r.org_key.map(|k| crate::organization::TierKey {
                        key_id: k.key_id,
                        tier: k.tier,
                        key: k.key,
                        valid_from: k.valid_from,
            ...
            proto::mesh_message::Payload::GlobalNodeAnnounce(r) => {
                let action = match r.action {
                    0 => crate::protocol::GlobalNodeAction::Add,
                    1 => crate::protocol::GlobalNodeAction::Remove,
                    2 => crate::protocol::GlobalNodeAction::UpdateKeyExchange,
                    _ => crate::protocol::GlobalNodeAction::Add,
                };
                ...
                let cert_chain = if r.cert_chain.is_empty() {
                    ...
                    serde_json::from_slice::<crate::cert::CertChain>(&r.cert_chain).ok()
                };
            ...
            proto::mesh_message::Payload::PeerBlocked(r) => Ok(MeshMessage::PeerBlocked {
                ...
                evidence_receipt: r.evidence_receipt.map(|er| crate::dht::AuditReceipt {
            ...
            proto::mesh_message::Payload::FindNodeResponse(r) => {
                use crate::dht::routing::{GeoInfo, NodeId, PeerContact};
            ...
            proto::mesh_message::Payload::NetworkPolicyUpdate(n) => {
                ...
                let blocked_nodes: Vec<crate::dht::BlockedNode> = policy
                    .blocked_nodes
                    .into_iter()
                    .map(|b| crate::dht::BlockedNode {
                        node_id: b.node_id,
                        ...
                        evidence_receipt: b.evidence_receipt.map(|er| crate::dht::AuditReceipt {
                            reporter_node_id: er.reporter_node_id,
                            target_node_id: er.target_node_id,
                            evidence_hash: er.evidence_hash,
                            evidence_type: er.evidence_type,
                    ...
                    .collect();
                let network_policy = crate::dht::NetworkPolicy {
            ...
            proto::mesh_message::Payload::GlobalNodeBlocklistUpdate(b) => {
                ...
                let blocked_nodes: Vec<crate::dht::BlockedNode> = blocklist
                    .blocked_nodes
                    .into_iter()
                    .map(|b| crate::dht::BlockedNode {
                        node_id: b.node_id,
                    ...
                    .collect();
                let global_blocklist = crate::dht::GlobalNodeBlocklist {
                    blocked_nodes,
            ...
            proto::mesh_message::Payload::AiBotListUpdate(a) => {
                ...
                let entries: Vec<crate::dht::AiBotEntry> = bot_list
                    .entries
                    .into_iter()
                    .map(|e| crate::dht::AiBotEntry {
                        pattern: e.pattern,
                        action: match e.action {
                            0 => crate::dht::BotAction::Add,
                            1 => crate::dht::BotAction::Remove,
                            _ => crate::dht::BotAction::Update,
                        },
                        source: e.source,
                    ...
                    .collect();
                let ai_bot_list = crate::dht::GlobalAiBotList {
                    entries,
            ...
            proto::mesh_message::Payload::WasmModuleAnnounce(r) => {
                Ok(MeshMessage::WasmModuleAnnounce {
                    ...
                    module_type: match r.module_type {
                        0 => crate::protocol::WasmModuleType::Plugin,
                        1 => crate::protocol::WasmModuleType::Serverless,
            ...
            proto::mesh_message::Payload::WasmModuleSyncResponse(r) => {
                Ok(MeshMessage::WasmModuleSyncResponse {
                    ...
                    modules: r
                        ...
                        .into_iter()
                        .map(|m| {
                            Ok(crate::protocol::WasmModuleInfo {
            ...
            proto::mesh_message::Payload::ServerlessFunctionAnnounce(r) => {
                Ok(MeshMessage::ServerlessFunctionAnnounce(
                    crate::protocol::ServerlessFunctionAnnounce {
            ...
            proto::mesh_message::Payload::ServerlessInvokeRequest(r) => {
                let permission_claim = r.permission_claim.as_ref().map(|pc| {
                    crate::protocol::ServerlessPermissionClaim {
                ...
                Ok(MeshMessage::ServerlessInvokeRequest(
                    crate::protocol::ServerlessInvokeRequest {
                        function_name: r.function_name.clone(),
            ...
            proto::mesh_message::Payload::ServerlessInvokeResponse(r) => Ok(
                ...
                let challenge_proof = match r.challenge_proof {
                    Some(cp) => match cp.proof {
                        Some(proto::ownership_challenge_proof::Proof::Http01Proof(h)) => {
                            crate::protocol::OwnershipChallengeProof::Http01 {
                                key_authorization: h.key_authorization,
                            }
                        }
                        Some(proto::ownership_challenge_proof::Proof::Dns01Proof(d)) => {
                            crate::protocol::OwnershipChallengeProof::Dns01 {
                                txt_record_value: d.txt_record_value,
                            }
                        }
                        None => {
            ...
            proto::mesh_message::Payload::SiteTlsCertSync(r) => {
                let certs = r
                    .certs
                    .into_iter()
                    .map(|c| crate::protocol::SiteTlsCertEntry {
                        site_id: c.site_id,
                ...
                Ok(MeshMessage::SiteTlsCertSync(
                    crate::protocol::SiteTlsCertSync {
                        site_id: r.site_id,
                        node_id: r.node_id,
                        timestamp: r.timestamp,
                        signature: r.signature,
            ...
            proto::mesh_message::Payload::SiteTlsCertRequest(r) => Ok(
                MeshMessage::SiteTlsCertRequest(crate::protocol::SiteTlsCertRequest {
                    site_id: r.site_id,
                    node_id: r.node_id,
                    timestamp: r.timestamp,
                    signature: r.signature,
            ...
            proto::mesh_message::Payload::SiteTlsCertResponse(r) => {
                ...
                Ok(MeshMessage::SiteTlsCertResponse(
                    crate::protocol::SiteTlsCertResponse {
                        site_id: r.site_id,
                        node_id: r.node_id,
                        timestamp: r.timestamp,
                        signature: r.signature,
            ...
            proto::mesh_message::Payload::OrgKeySignRequest(r) => {
                let org_public_key = r.org_public_key.ok_or(ProtocolError::MissingPayload)?;
                Ok(MeshMessage::OrgKeySignRequest {
                    request_id: r.request_id.into(),
                    org_id: r.org_id.into(),
                    org_public_key: crate::organization::OrgPublicKey {
                        org_id: org_public_key.org_id,
                        key_id: org_public_key.key_id,
                        public_key: org_public_key.public_key,
                        created_at: org_public_key.created_at,
                        issued_by: org_public_key.issued_by,
            ...
            proto::mesh_message::Payload::OrgKeySignResponse(r) => {
                Ok(MeshMessage::OrgKeySignResponse {
                    request_id: r.request_id.into(),
                    org_id: r.org_id.into(),
                    signature: r.signature,
                    signer_node_id: r.signer_node_id.into(),
                    timestamp: r.timestamp,
                })
            }
            proto::mesh_message::Payload::Raft(r) => {
                let msg_type = match r.msg_type {
                    0 => crate::protocol::RaftMsgType::VoteRequest,
                    1 => crate::protocol::RaftMsgType::VoteResponse,
                    2 => crate::protocol::RaftMsgType::AppendEntries,
                    3 => crate::protocol::RaftMsgType::AppendEntriesResponse,
                    4 => crate::protocol::RaftMsgType::InstallSnapshot,
                    5 => crate::protocol::RaftMsgType::InstallSnapshotResponse,
                    _ => crate::protocol::RaftMsgType::VoteRequest,
                };
                Ok(MeshMessage::Raft {
            ...
            proto::mesh_message::Payload::ConsistentReadRequest(r) => {
                Ok(MeshMessage::ConsistentReadRequest {
                    request_id: r.request_id.into(),
                    namespace: crate::raft::state_machine::Namespace::from_str(&r.namespace)
