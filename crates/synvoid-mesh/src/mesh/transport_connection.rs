...
use crate::transport::{MeshPeerConnection, MeshTransport, MeshTransportError, MAX_MESSAGE_SIZE};
use std::sync::Arc;
...
impl MeshTransport {
            ...
            tokio::select! {
                ...
                _ = cleanup_interval.tick() => {
                    ...
                    if let Some(partition) = topology.check_network_partition().await {
                        match partition {
                            crate::topology::NetworkPartitionState::Isolated { .. } => {
                                tracing::warn!("Network partition detected: isolated from all peers");
                            }
                            crate::topology::NetworkPartitionState::DisconnectedFromGlobal { .. } => {
                                tracing::warn!("Network partition detected: disconnected from global nodes");
                            }
                            crate::topology::NetworkPartitionState::Degraded { .. } => {
    ...
    pub(crate) async fn handle_seed_list_response(
        ...
        genesis_org_id: Option<crate::protocol::ArcStr>,
    ) {
        tracing::info!(
    ...
    pub(crate) async fn perform_auto_slash(&self) {
        ...
        for node_id in auth_failures {
            ...
            stake_mgr.slash_node(
                &node_id,
                crate::dht::stake::SlashReason::RepeatedMisbehavior,
                "auto-slash",
            );
        }
