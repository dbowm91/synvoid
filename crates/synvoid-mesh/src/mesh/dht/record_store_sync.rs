...
impl RecordStoreManager {
    ...
    pub fn create_snapshot_response(
        ...
        let records: Vec<DhtRecord> = self
            ...
            .take(crate::transport::MAX_SNAPSHOT_RECORDS)
            .collect();
    ...
    pub fn apply_snapshot(
        ...
        let ingress_ctx = crate::dht::signed::DhtRecordIngressContext::new_remote(
            ...
            crate::dht::signed::IngressPath::SnapshotSync,
        );

        let reputation = if is_verified { 100 } else { 0 };
        let mut applied = 0;
        for record in records {
            if self.store_record_from_ingress(record, &ingress_ctx, reputation) {
                applied += 1;
            }
        }
    ...
    pub fn verify_and_apply_snapshot(
        ...
        for record in records {
            ...
            let record_type = dht_key
                ...
                .unwrap_or(crate::dht::signed::SignedRecordType::NodeInfo);
            ...
            if !verified {
                ...
                if let Some(ref stake_mgr) = self.routing_state.read().stake_manager {
                    stake_mgr.submit_global_slash_vote(
                        ...
                        crate::dht::stake::SlashReason::InvalidRecordSignature,
                    );
                }
                continue;
            }
...
mod tests {
    ...
    fn test_record_type_derivation_from_dht_key() {
        let test_cases = vec![
            ...
            (
                "org:test",
                crate::dht::signed::SignedRecordType::Organization,
            ),
            (
                "upstream:example.com",
                crate::dht::signed::SignedRecordType::Upstream,
            ),
            (
                "verified_upstream:example.com",
                crate::dht::signed::SignedRecordType::VerifiedUpstream,
            ),
            (
                "dns_record:example.com:www",
                crate::dht::signed::SignedRecordType::DnsRecord,
            ),
            (
                "tier_claim:my-org",
                crate::dht::signed::SignedRecordType::TierClaim,
            ),
            (
                "global_node_heartbeat:node1",
                crate::dht::signed::SignedRecordType::GlobalNodeHeartbeat,
            ),
            (
                "node_health:node1",
                crate::dht::signed::SignedRecordType::NodeHealth,
            ),
        ];

        for (key, expected_type) in test_cases {
            let dht_key = crate::dht::keys::DhtKey::from_str(key);
    ...
    fn test_create_sync_request_signed_when_mesh_signer_present() {
        ...
        let store =
            build_test_record_store(Some(crate::protocol::MeshMessageSigner::new(signing_key)));
        ...
        match req.unwrap() {
            MeshMessage::DhtSyncRequest {
                ...
                let verifier = crate::protocol::MeshMessageSigner::new([9u8; 32]);
                let content = crate::dht::signed::get_sync_request_signable_content(
                    &request_id,
                    &node_id,
                    from_version,
