...
// NOTE (Phase 34): the `synvoid-config` site conversion
// (`upstream_tls_from_site_config`) now lives in
// `synvoid_upstream::tls_adapter`, next to upstream selection/health
// checking. This module keeps only the generic transport policy
// (`UpstreamTlsConfig`), root/CA loading, and rustls `ClientConfig`
// construction so the transport layer has no SynVoid config dependency.
