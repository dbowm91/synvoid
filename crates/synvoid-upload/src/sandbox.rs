...
pub struct SandboxConfig {
    ...
    pub sandbox_level: synvoid_platform::SandboxLevel,
}
...
impl Default for SandboxConfig {
    fn default() -> Self {
        Self {
            ...
            sandbox_level: synvoid_platform::SandboxLevel::Off,
        }
    }
}
...
impl SandboxConfig {
    ...
    pub fn with_sandbox_level(mut self, level: synvoid_platform::SandboxLevel) -> Self {
    ...
    pub fn apply_platform_sandbox(&self) -> Option<synvoid_platform::ProcessSandbox> {
        if self.sandbox_level == synvoid_platform::SandboxLevel::Off {
            return None;
        }

        if !synvoid_platform::is_sandbox_supported() {
            tracing::warn!(
        ...
        let paths = synvoid_platform::SandboxPaths::new()
            .add_read_path(&self.sandbox_dir)
            .add_write_path(&self.quarantine_dir);

        match synvoid_platform::ProcessSandbox::with_paths(self.sandbox_level, paths) {
            Ok(sandbox) => {
                tracing::info!(
