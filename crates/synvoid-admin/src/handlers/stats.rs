...
pub async fn get_summary<S: AdminStateProvider>(
...
pub async fn get_sites_stats<S: AdminStateProvider>(
    State(state): State<Arc<S>>,
    _auth: OptionalAuth,
...
pub async fn get_metrics_history<S: AdminStateProvider>(
    State(state): State<Arc<S>>,
    _auth: OptionalAuth,
...
pub async fn get_attack_stats<S: AdminStateProvider>(
    State(state): State<Arc<S>>,
    _auth: OptionalAuth,
...
pub async fn get_cache_stats<S: AdminStateProvider>(
    State(state): State<Arc<S>>,
    _auth: OptionalAuth,
    ...
    let (static_cache_hits, static_cache_misses) = state
        .process_manager()
        .map(|pm| pm.get_cpu_worker_cache_stats())
        .unwrap_or((0, 0));
    ...
    let cpu_offload_pool_stats = get_global_async_cpu_offload_stats();
    let cpu_worker_stats = state
        .process_manager()
        .map(|pm| pm.get_cpu_worker_cpu_offload_stats())
        .unwrap_or_default();
...
use synvoid_metrics::bandwidth::{get_global_bandwidth_tracker, BandwidthPayload};
...
pub async fn get_bandwidth<S: AdminStateProvider>(
...
pub async fn get_request_logs<S: AdminStateProvider>(
    State(state): State<Arc<S>>,
    _auth: OptionalAuth,
