...
///
/// Kept as a named constant so runtime gates, config docs, and guard tests
/// compare against one canonical value instead of duplicated literals.
...
impl UnsafeNativeExtensionConfig {
    ...
    pub fn validate_for_load(
        ...
        if self.is_production() {
            ...
            let expected_ack = RISK_ACKNOWLEDGEMENT;
...
// ─── External-host seam (deliberately non-executing) ─────────────────────────
//
// Phase 28 does NOT implement out-of-process native hosting and does NOT
// provide a generic remote-execution API. The previous `ExternalPluginClient`
// placeholder claimed a `filter_request` execution method without an
// implementation; that over-claim is removed here.
//
// The concrete follow-up seam lives in `crate::external_seam`: versioned
// request/response DTOs, size/deadline/concurrency bounds, and explicit
// non-goals (no inherited secrets, no arbitrary path/symbol operations).
// Until an external host backend exists, in-process loading via `load_plugin`
// is the only execution path, and it remains explicitly unsafe (see below).
//
...
/// Additionally, every router derived from the extension carries its own
/// `Arc<Library>` keep-alive inside its middleware layer, so cloned routers
/// keep the library mapped even after the backend unloads the named
/// generation (old generations drain via `Arc` reference counting).
///
/// Every path reaches all applicable gates (production gate → path validation
/// → hash verification) before the first `Library::new` call.
///
/// # Safety
///
/// This function calls into native code via FFI. Rust panics in the native
/// `create_router` function are caught via `catch_unwind` so a Rust panic
/// does not unwind across the FFI boundary. `catch_unwind` is NOT a sandbox:
/// it catches Rust panics only. Arbitrary undefined behavior, memory
/// corruption, or process-aborting faults in native code can still corrupt or
/// crash the host process. In-process native code is and remains unsandboxed.
pub fn load_plugin(
    path: &Path,
    allowed_dirs: &[String],
    ...
    match load_result {
        Ok(Ok(ext)) => {
            crate::metrics::record_unsafe_native_extension_loaded(&ext.name);
        ...
        Ok(Err(UnsafeNativePluginError::AbiMismatch { plugin, expected })) => {
            crate::metrics::record_unsafe_native_extension_load_failed(&name);
...
/// library. The caller must ensure `catch_unwind` wraps this so a Rust panic
/// does not unwind across the FFI boundary. This does not contain arbitrary
/// native UB: only Rust-panic unwinding is intercepted.
unsafe fn load_native_library(
    ...
    //
    // The middleware also captures an `Arc<Library>` keep-alive clone, so
    // every cloned router keeps the shared library mapped for as long as the
    // router itself is alive (unload drops the backend's handle, but
    // outstanding routers still hold the library via this layer).
    let extension_name = name.to_owned();
    let library_keep_alive = lib.clone();
    let metrics_layer = axum::Router::new().layer(axum::middleware::from_fn(
        move |req: axum::http::Request<axum::body::Body>, next: axum::middleware::Next| {
            let name = extension_name.clone();
            let _keep_alive = library_keep_alive.clone();
            async move {
                crate::metrics::record_unsafe_native_extension_request(&name);
                let _hold = _keep_alive;
                next.run(req).await
            }
        },
    ));
...
mod tests {
    ...
    fn test_load_failure_records_metric() {
        crate::metrics::record_unsafe_native_extension_load_failed("test_metric_fail");
    }
    ...
    fn test_load_success_records_metric() {
        crate::metrics::record_unsafe_native_extension_loaded("test_metric_success");
    }
