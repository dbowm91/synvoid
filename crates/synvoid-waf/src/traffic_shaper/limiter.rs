...
use synvoid_config::ConnectionLimitsConfig;
            ...
            if let Some(counter) = self.site_total_connections.get(&token.site_id) {
                ...
                if prev == Ok(1) {
                    ...
                    self.site_total_connections
                        .remove_if(&token.site_id, |_, v| v.load(Ordering::Acquire) == 0);
                }
            }
        }

        {
            if let Some(counter) = self.ip_connections.get(&token.client_ip) {
                ...
                if prev == Ok(1) {
                    ...
                    let removed = self
                        .ip_connections
                        .remove_if(&token.client_ip, |_, v| v.load(Ordering::Acquire) == 0);
                    if removed.is_some() {
...

        {
            if let Some(site_ips) = self.site_connections.get(&token.site_id) {
                if let Some(counter) = site_ips.get(&token.client_ip) {
                    ...
                    if prev == Ok(1) {
                        drop(counter);
                        site_ips.remove_if(&token.client_ip, |_, v| v.load(Ordering::Acquire) == 0);
                    }
                }
