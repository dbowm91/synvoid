...
impl SuspiciousWordTracker {
    pub fn new(config: synvoid_config::SuspiciousWordsConfig) -> Arc<Self> {
...
impl UpstreamErrorTracker {
    pub fn new(config: synvoid_config::UpstreamErrorsConfig) -> Arc<Self> {
        Arc::new(Self {
            store: Arc::new(RwLock::new(HashMap::new())),
            config,
    ...
    pub fn get_config(&self) -> &synvoid_config::UpstreamErrorsConfig {
        &self.config
    }
