...
pub use config::{
    ClientPortMapping, ReconnectConfig, TransportType, VpnClientConfig,
    WireGuardClientTransportConfig,
};
...
use synvoid_platform::{self as platform, Platform};
use synvoid_tunnel::quic::framing::{read_message_default, write_message};
use synvoid_tunnel::quic::messages::{DatagramCapabilities, PortMapping, TunnelMessage};
use synvoid_tunnel::quic::validation::JitteredBackoff;
use synvoid_tunnel::quic::QuicRuntime;
use synvoid_tunnel::wireguard::WireGuardRuntime;
use synvoid_tunnel::TunnelTransport;
...
impl VpnClient {
    pub fn new(config: VpnClientConfig) -> Result<Self, Box<dyn std::error::Error + Send + Sync>> {
        ...
        if config.transport == TransportType::WireGuard
        ...
        if config.transport == TransportType::WireGuard && platform::is_admin_required_for_tun() {
            tracing::info!("WireGuard requires administrator/root privileges on this platform");
        }
    ...
    async fn connect_quic(&mut self) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
        ...
        let server_name = self
            ...
            .server_name
            .as_deref()
            .unwrap_or(&self.config.server_host);
        ...
        let port_mappings: HashMap<String, PortMapping> = self
            .config
            .port_mappings
            .iter()
            .map(|m| {
                ...
                (
                    m.identifier(),
                    PortMapping::new(m.remote_port, &protocol_str).with_upstream(
                        m.upstream_host.as_deref().unwrap_or("127.0.0.1"),
                        m.remote_port,
                    ),
                )
            })
        ...
        match response {
            TunnelMessage::HelloAck {
                ...
                let access_level_str = access_level
                    .clone()
            ...
            Some(VpnConnection::Quic { session }) => {
                for mapping in &self.config.port_mappings {
                    let local_mapping = LocalPortMapping {
                        local_addr: format!(
                            "{}:{}",
                            self.config.local_bind_host, mapping.local_port
                        )
                        .parse()
    ...
    pub fn is_tun_available() -> bool {
        synvoid_tunnel::tun::is_tun_available()
    }
