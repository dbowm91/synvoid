...
pub struct YaraScanner {
    ...
    pub(crate) queue_semaphore: Arc<tokio::sync::Semaphore>,
...
impl YaraScanner {
    pub fn new(rules_source: YaraRulesSource) -> Result<Self, YaraError> {
        Self::with_timeout(rules_source, 30000, 4, 64, 1000)
    }
    ...
    ///
    /// Note: upload archive depth/size policy intentionally lives in
    /// `synvoid-upload`, not here. This engine owns only YARA-generic bounds
    /// (scan concurrency, queue depth, timeouts, rule-source limits).
    pub fn with_timeout(
        rules_source: YaraRulesSource,
        timeout_ms: u64,
...
/// Syntax-validate rule text without retaining it.
///
/// This is the narrow validation capability mesh distribution uses via
/// injection: mesh itself never links `yara-x`; composition provides this
/// function (or the [`crate::executor::YaraExecutor`] trait) from the
/// execution boundary.
pub fn validate_rules_syntax(rules: &str) -> Result<(), YaraError> {
    // Structural pre-checks mirror the mesh distribution limits so obviously
    // oversized rule sets fail fast without invoking the compiler.
    const MAX_RULES_BYTES: usize = 2 * 1024 * 1024;
    if rules.len() > MAX_RULES_BYTES {
        return Err(YaraError::CompilationError(format!(
            "rules size {} exceeds limit {}",
            rules.len(),
            MAX_RULES_BYTES
        )));
    }
    if !rules.contains("rule ") {
        return Err(YaraError::CompilationError(
            "rules must contain at least one 'rule' declaration".to_string(),
        ));
    }
    yara_x::compile(rules)
        .map(|_| ())
        .map_err(|e| YaraError::CompilationError(e.to_string()))
}

/// Constant-time digest check for jail/loader paths.
///
/// `expected_hex` must be 64 lowercase hex chars (SHA-256). Malformed input
/// returns `false` without leaking timing about the digest itself.
pub fn verify_content_digest(data: &[u8], expected_hex: &str) -> bool {
    use sha2::{Digest, Sha256};
...
impl YaraScanner {
    ...
    /// Backwards-compatible factory preserving the pre-Phase-26 argument shape.
    ///
    /// `archive_max_depth`/`archive_max_size` are upload archive-inspection
    /// policy and are intentionally ignored here; they live in
    /// `synvoid-upload`. Retained so existing call sites keep compiling while
    /// they migrate to [`YaraScanner::with_scan_executor`].
    pub fn with_timeout_compat(
        rules_source: YaraRulesSource,
        timeout_ms: u64,
