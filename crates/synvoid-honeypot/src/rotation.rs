...
use crate::responses::{HoneypotResponder, HoneypotResponderRegistry};
            ...
            let ports: Vec<StablePort> = config
                ...
                .map(|sp| {
                    let responder = registry
                        .get_or_default(&sp.service)
                        .unwrap_or_else(|| Arc::new(crate::responders::StaticResponder::http()));
                    StablePort {
                        port: sp.port,
    ...
    fn get_cycle_services(&self) -> Vec<Arc<dyn HoneypotResponder>> {
        vec![
            self.responder_registry
                .get_or_default("ssh")
                .unwrap_or_else(|| {
                    Arc::new(crate::responders::VulnerableAppResponder::ubuntu_ssh())
                }),
            self.responder_registry
                .get_or_default("http")
                .unwrap_or_else(
                    || Arc::new(crate::responders::VulnerableAppResponder::wordpress()),
                ),
            self.responder_registry
                .get_or_default("mysql")
                .unwrap_or_else(|| Arc::new(crate::responders::VulnerableAppResponder::mysql())),
            self.responder_registry
                .get_or_default("redis")
                .unwrap_or_else(|| Arc::new(crate::responders::VulnerableAppResponder::redis())),
            self.responder_registry
                .get_or_default("elasticsearch")
                .unwrap_or_else(|| {
                    Arc::new(crate::responders::VulnerableAppResponder::elasticsearch())
                }),
            self.responder_registry
                .get_or_default("docker")
                .unwrap_or_else(|| {
                    Arc::new(crate::responders::VulnerableAppResponder::docker_api())
                }),
            self.responder_registry
                .get_or_default("jenkins")
                .unwrap_or_else(|| Arc::new(crate::responders::VulnerableAppResponder::jenkins())),
            self.responder_registry
                .get_or_default("tomcat")
                .unwrap_or_else(|| Arc::new(crate::responders::VulnerableAppResponder::tomcat())),
        ]
    }
...
mod tests {
    ...
    fn test_responder_registry_get_or_default() {
        let mut registry = crate::responses::HoneypotResponderRegistry::new();

        // Empty registry should return None
        assert!(registry.get_or_default("ssh").is_none());

        // After register, should find
        registry.register(Arc::new(crate::responders::StaticResponder::ssh()));
