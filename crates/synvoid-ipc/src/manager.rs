...
pub struct ProcessManagerConfig {
    ...
    pub control_api_tls: Option<synvoid_tls::InternalTlsConfig>,
...
impl Default for ProcessManagerConfig {
    fn default() -> Self {
        ...
        Self {
            ...
            supervisor_socket_path: crate::get_secure_socket_path("supervisor.sock"),
            log_level: None,
            pre_spawn_workers: 0,
...
pub struct ProcessManager {
    ...
    dynamic_config: Arc<PLRwLock<synvoid_config::ProcessManagerConfig>>,
    ...
    _block_store: Option<Arc<dyn std::any::Any + Send + Sync>>,
...
impl ProcessManager {
    pub fn new(
        config: ProcessManagerConfig,
        block_store: Option<Arc<dyn std::any::Any + Send + Sync>>,
        ...
        let dynamic_config = synvoid_config::ProcessManagerConfig {
    ...
    pub fn get_config(&self) -> synvoid_config::ProcessManagerConfig {
        let dynamic = self.dynamic_config.read();
        synvoid_config::ProcessManagerConfig {
    ...
    fn spawn_unified_server_worker_with_id(&self, id: WorkerId) -> std::io::Result<WorkerId> {
        ...
        let enable_shared_port_mode =
            total_workers > 1 && synvoid_platform::is_reuse_port_supported();
    ...
    async fn drain_worker_async(
        ...
        if let Some(ipc) = ipc {
            let drain_id = synvoid_utils::safe_unix_duration().as_millis() as u64;
    ...
    pub fn handle_heartbeat(&self, worker_id: WorkerId, metrics: WorkerMetricsPayload) {
        ...
        if let Some(evt) = event {
            if let Err(e) = self.event_tx.try_send(evt) {
                tracing::debug!("dropped_process_event");
    ...
    pub fn trigger_blocklist_persist(&self) {
        // BlockStore not available in extracted crate
    }

    pub async fn broadcast_rule_patterns_update(
        &self,
        version: String,
        patterns: Vec<crate::ipc::RulePatternData>,
    ) {
    ...
    pub async fn broadcast_threat_feed_update(
        &self,
        indicators: Vec<crate::ipc::ThreatIndicatorData>,
        version: u64,
    ) {
