...
use crate::ipc::{IpcListener, IpcStream, IpcTransport};
use crate::process::{ProcessControl, Signal, SignalHandler};
use crate::socket::{OwnedTcpListener, OwnedTcpStream, SocketHandoffError, SocketInfo, SocketType};
...
impl crate::socket::SocketHandle for UnixSocketHandle {
    fn as_tcp_listener(&self) -> io::Result<TcpListener> {
        let fd_dup = nix::unistd::dup(self.fd).map_err(io::Error::other)?;
...
impl crate::socket::SocketFDPassing for UnixSocketFDPassing {
    ...
    fn send_sockets(&self, handles: &[Self::Handle]) -> Result<(), SocketHandoffError> {
        let stream = self
            .stream
            .as_ref()
            .ok_or(SocketHandoffError::NotConnected)?;
...
pub fn create_listening_socket_unix(
    port: u16,
    reuse_port: bool,
...
pub fn create_listening_socket_v6_unix(
    port: u16,
    reuse_port: bool,
) -> Result<SocketInfo, PlatformError> {
    use nix::sys::socket::{AddressFamily, SockFlag, SockProtocol, SockType};
...
impl IpcListener for UnixIpcListener {
    ...
    fn bind(path: &Path) -> Result<Self, PlatformError> {
        ...
        let listener = UnixListener::bind(path).map_err(|e| PlatformError::Ipc(e.to_string()))?;
...
impl IpcStream for UnixIpcStream {
    fn connect(path: &Path) -> Result<Self, PlatformError> {
        let stream = UnixStream::connect(path).map_err(|e| PlatformError::Ipc(e.to_string()))?;
