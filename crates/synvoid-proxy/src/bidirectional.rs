...
use synvoid_waf::attack_detection::streaming::StreamingWafCore;
                ...
                Ok(n) => {
                    ...
                    if let Some(ref scanner) = waf_scanner {
                        ...
                        match scanner.scan_chunk(chunk) {
                            synvoid_waf::attack_detection::streaming::StreamingWafDecision::Block(status, reason) => {
                                return Err(ProxyError::WafBlock(status, reason));
                            }
                            synvoid_waf::attack_detection::streaming::StreamingWafDecision::Continue => {}
                        }
                    }
