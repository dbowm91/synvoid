...
proptest! {
    ...
    fn dns_encode_parse_roundtrip(name in "[a-z]{1,10}(\\.[a-z]{1,10}){0,3}") {
        let encoded = synvoid_dns::wire::encode_name(&name);
        let parsed = synvoid_dns::wire::parse_query_name(&encoded, 0);
    ...
    fn dns_message_id_preserved(id in 0u16..=65535u16) {
        ...
        let response = synvoid_dns::wire::build_response_header(id, flags, 1, 0, 0, 0);
        let parsed_id = synvoid_dns::wire::get_message_id(&response);
    ...
    fn dns_build_question_valid(
        ...
        let question = synvoid_dns::wire::build_question(&name, qtype, 1);
    ...
    fn dns_error_response_preserves_id(id in 0u16..=65535u16) {
        ...
        if let Some(response) = synvoid_dns::wire::build_error_response(&query, 3) {
            let parsed_id = synvoid_dns::wire::get_message_id(&response);
            prop_assert_eq!(parsed_id, Some(id));
        }
    }
