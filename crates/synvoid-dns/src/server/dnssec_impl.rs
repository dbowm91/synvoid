...
impl DnsServer {
    ...
    pub(super) fn build_dnskey_records(zone: &Zone) -> Vec<DnsZoneRecord> {
        ...
        if let Some(ref ksk) = zone.ksk_key {
            let dnskey_rdata = crate::dnssec::compute_dnskey(ksk);
        ...
        if let Some(ref zsk) = zone.zsk_key {
            let dnskey_rdata = crate::dnssec::compute_dnskey(zsk);
            records.push(DnsZoneRecord {
                name: "@".to_string(),
                record_type: RecordType::DNSKEY,
                value: hex::encode(&dnskey_rdata),
    ...
    pub(super) fn build_ds_records(ksk: &crate::dnssec::ZoneSigningKey) -> Vec<DnsZoneRecord> {
        let mut records = Vec::new();

        if let Ok(ds_data) =
            crate::dnssec::create_ds_record(ksk, crate::dnssec::DsDigestType::Sha256)
        {
            records.push(DnsZoneRecord {
                name: "@".to_string(),
    ...
    pub(super) fn build_cds_records(ksk: &crate::dnssec::ZoneSigningKey) -> Vec<DnsZoneRecord> {
        let mut records = Vec::new();
        ...
        if let Ok(ds_data_sha1) =
            crate::dnssec::create_ds_record(ksk, crate::dnssec::DsDigestType::Sha1)
        {
            records.push(DnsZoneRecord {
                name: "@".to_string(),
                record_type: RecordType::CDS,
    ...
    pub fn export_ds_records(&self, zone_name: &str) -> Result<Vec<DsRecordExport>, String> {
        ...
        for digest_type in &[
            crate::dnssec::DsDigestType::Sha256,
            crate::dnssec::DsDigestType::Sha1,
        ] {
            if let Ok(ds_data) = crate::dnssec::create_ds_record(ksk, *digest_type) {
                if ds_data.len() >= 4 {
    ...
    pub(super) fn build_cdnskey_records(zone: &Zone) -> Vec<DnsZoneRecord> {
        ...
        if let Some(ref ksk) = zone.ksk_key {
            let cdnskey_rdata = crate::dnssec::compute_dnskey(ksk);
            records.push(DnsZoneRecord {
                name: "@".to_string(),
    ...
    pub(super) fn build_nsec3_records(
        ...
        let closest_hash = crate::dnssec::hash_name_nsec3(&closest_encloser, nsec3param);
        let closest_hash_b32 = crate::dnssec::create_nsec3_owner_name(zone_origin, &closest_hash);

        let wildcard_name = format!("*.{}", closest_encloser);
        let wildcard_hash = crate::dnssec::hash_name_nsec3(&wildcard_name, nsec3param);
        let wildcard_hash_b32 = crate::dnssec::create_nsec3_owner_name(zone_origin, &wildcard_hash);
        ...
        let next_closer_hash = crate::dnssec::hash_name_nsec3(&next_closer, nsec3param);
        let next_closer_hash_b32 =
            crate::dnssec::create_nsec3_owner_name(zone_origin, &next_closer_hash);

        let wildcard_types = vec![1, 2, 5, 6, 16, 28, 33];

        let wildcard_nsec3 = crate::dnssec::create_nsec3_record(
            &wildcard_hash_b32,
            &next_closer_hash_b32,
            nsec3param,
            &wildcard_types,
        );

        let closest_nsec3 = crate::dnssec::create_nsec3_record(
            &closest_hash_b32,
            &wildcard_hash_b32,
            nsec3param,
            &wildcard_types,
        ...
        let next_closer_nsec3 = crate::dnssec::create_nsec3_record(
            &next_closer_hash_b32,
            &closest_hash_b32,
            nsec3param,
            &[],
        ...
        records.push(DnsZoneRecord {
            name: next_closer_hash_b32,
            record_type: RecordType::NSEC3,
            value: hex::encode(&next_closer_nsec3),
            ttl: zone.dnskey_ttl.unwrap_or(3600),
            priority: None,
        });
        ...
        if let Some(soa_record) = zone.records.get(&("@".to_string(), RecordType::SOA)) {
            if !soa_record.is_empty() {
                let soa_hash = crate::dnssec::hash_name_nsec3(zone_origin, nsec3param);
                let soa_hash_b32 = crate::dnssec::create_nsec3_owner_name(zone_origin, &soa_hash);

                let soa_types = vec![1, 2, 5, 6, 16, 28, 33];
                let soa_nsec3 = crate::dnssec::create_nsec3_record(
                    &soa_hash_b32,
                    &closest_hash_b32,
                    nsec3param,
                    &soa_types,
    ...
    pub(super) fn build_nsec_records(
        ...
        let next_name = crate::dnssec::find_next_name_in_zone(zone, &qname_lower)
        ...
        let nsec_rdata = crate::dnssec::create_nsec_record(&qname_lower, &next_name, &types);
    ...
    pub(super) fn build_nsec3_nodata(
        ...
        let qname_hash = crate::dnssec::hash_name_nsec3(qname, nsec3param);
        let qname_hash_b32 = crate::dnssec::create_nsec3_owner_name(zone_origin, &qname_hash);
        ...
        let next_domain = if qname_hash_b32 == soa_hash_b32 {
            let wildcard_name = format!("*.{}", zone_origin);
            let wildcard_hash = crate::dnssec::hash_name_nsec3(&wildcard_name, nsec3param);
            crate::dnssec::create_nsec3_owner_name(zone_origin, &wildcard_hash)
        } else {
            soa_hash_b32.clone()
        };

        let nsec3_rdata =
            crate::dnssec::create_nsec3_record(&qname_hash_b32, &next_domain, nsec3param, &types);

        records.push(DnsZoneRecord {
            name: qname_hash_b32.clone(),
            record_type: RecordType::NSEC3,
        ...
        if let Some(soa_record) = zone.records.get(&("@".to_string(), RecordType::SOA)) {
            if !soa_record.is_empty() {
                ...
                let soa_next = if soa_hash_b32 == qname_hash_b32 {
                    let wildcard_name = format!("*.{}", zone_origin);
                    let wildcard_hash = crate::dnssec::hash_name_nsec3(&wildcard_name, nsec3param);
                    crate::dnssec::create_nsec3_owner_name(zone_origin, &wildcard_hash)
                } else {
                    qname_hash_b32.clone()
                };
    ...
    pub(super) fn build_nsec3param_record(zone: &Zone) -> Option<DnsZoneRecord> {
        ...
        let nsec3param_data = crate::dnssec::create_nsec3param_record(nsec3param);
    ...
    pub(super) fn create_signed_rrsig(
        ...
        signer_name: &str,
        key: &crate::dnssec::ZoneSigningKey,
    ) -> Vec<u8> {
        let labels = crate::dnssec::count_labels(&record.name);

        let canonical_rdata = crate::dnssec::canonical_rdata(
        ...
        let signature = match crate::dnssec::sign_data(&canonical_msg, key) {
            Ok(sig) => sig,
            Err(e) => {
        ...
        if let Some(dnssec_manager) = dnssec {
            ...
            tokio::spawn(async move {
                ...
                loop {
                    ...
                    let config = crate::dnssec::KeyRotationConfig::default();
