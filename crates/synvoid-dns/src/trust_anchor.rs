...
impl TrustAnchorManager {
        ...
        if let Some(anchor) = anchors.get_mut(&key_id) {
            match anchor.state {
                TrustAnchorState::Seen => {
                    let computed_digest = match crate::dnssec::compute_ds_digest(
                        digest_type,
                        257,
                        3,
                        algorithm,
    ...
    fn parse_dnskey_record_line(line: &str) -> Option<(u16, u8, Vec<u8>)> {
        ...
        let key_tag = crate::dnssec::calculate_key_tag(257, 3, algorithm, &public_key);
...
mod tests {
    ...
    fn test_trust_anchor_check_pending() {
        ...
        let digest = crate::dnssec::compute_ds_digest(2, 257, 3, 8, &public_key)
    ...
    fn test_ds_digest_sha1() {
        ...
        let digest = crate::dnssec::compute_ds_digest(1, 257, 3, 8, &public_key);
        assert!(digest.is_ok());
    ...
    fn test_ds_digest_sha384() {
        ...
        let digest = crate::dnssec::compute_ds_digest(4, 257, 3, 8, &public_key);
        assert!(digest.is_ok());
        let digest = digest.unwrap();
    ...
    fn test_digest_verification_roundtrip() {
        ...
        let verified = crate::dnssec::verify_ds_digest(2, 257, 3, 8, &public_key, &digest)
    ...
    fn test_public_key_change_rejection() {
        ...
        let key_tag = crate::dnssec::calculate_key_tag(257, 3, 8, &public_key1);
