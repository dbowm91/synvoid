...
`read_exact_with_timeout()` call sites
...
applied at the future level. Per-message reads now use `read_exact_with_timeout()` directly, applying the timeout to the actual I/O operation.
...
- `read_exact_with_timeout()` — deadline-aware reads replacing the removed `apply_read_timeouts` wrapper
...
When `mesh.enabled = false` or mesh config is absent, `MeshInit::disabled()` returns no runtime resources. No topology, routing, transport, DNS, YARA, or DHT objects are created. `build_mesh_supervision_policy()` returns `None` — no supervision pipeline, observer, coordinator, startup task, or decision channel exists.
...
### Required vs Optional Startup Paths (updated Iteration 85)

- **Required mesh**: `start_mesh_generation()` is awaited inline before the worker sends its ready signal. Worker startup blocks on mesh readiness. On failure: status transitions once, `DirectCause` set, no ready message, coordinated shutdown begins. No coordinator round-trip needed.
- **Optional mesh**: Mesh starts asynchronously; worker proceeds without waiting for mesh to reach `Running` state.
- **Disabled mesh**: No mesh startup occurs. No pipeline created.
...
### start_mesh_generation() (Iteration 85)

`start_mesh_generation()` returns facts only: `Result<(), MeshFailureCause>`. Caller is responsible for transitioning `WorkerMeshStatus` before/after the call. No status mutation occurs inside the helper. The coordinator handles runtime event transitions; required startup failure transitions status directly at the composition root.
...
Support tasks (DNS verification, YARA broadcast) are now registered AFTER mesh startup succeeds, not before. This prevents support tasks from starting before the mesh transport is ready.
...
| 87 | DHT routing initialization moved into MeshTransport transactional startup (Phase 5.5). Removed `dht_routing_init` one-shot worker task; routing manager owned exclusively by MeshTransport. Generation-scoped worker support bundles via `MeshGenerationSupport`. |
| 88 | DHT initialization reordered to Phase 3.5 (before any peer connections). Cooperative support teardown with `stop_mesh_generation_support()` and `cancel_then_join_tasks()`. YARA bridge task removed; `run_yara_broadcast_loop()` accepts worker and generation shutdown directly. DHT degraded reporting reflects actual initialization state. |
