...
- **QuicTunnel:** Proxying through a specialized QUIC tunnel. URL parsing is unified via `Router::parse_quictunnel_url()` at `src/router.rs:512-532` at both location and site levels.
...
- **PeakEwma:** Cost-based selection using exponential weighted moving average of latency `(conn + 1) * (latency + 1)` at `src/upstream/pool.rs:48-57` (formula at `src/upstream/pool.rs:517-521`).
