...
- **Site-level tracking:** Per-site connection counting via direct `try_acquire_with_limits()` calls on a global `ConnectionLimiter` with `site_id` parameter (applies limits by site_id internally)
