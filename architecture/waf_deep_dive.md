...
- **Volumetric Mitigation:** Protects against SYN floods, UDP floods, and connection exhaustion.
- **Rate Limiting:** Per-IP and global rate limits to prevent brute-force attacks and resource exhaustion.
- **eBPF Integration:** For high-volume attacks, SynVoid can utilize eBPF (on Linux) to block malicious traffic at the kernel level before it reaches userspace.
- **ASN & GeoIP Blocking:** Allows for blocking or rate-limiting traffic based on source country or Autonomous System Number.
...
- **JWT & XXE Detection:** Specialized detectors for JSON Web Tokens and XML External Entities.
...
- **Zero-Copy Inspection:** Minimizes data copying during inspection.
- **Parallel Processing:** Different layers of the WAF can execute concurrently where possible.
- **Aho-Corasick & Regex:** High-performance pattern matching engines are used for rule evaluation.
