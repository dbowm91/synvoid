...
4. **~~Per-target stale suppression is in-memory only~~**: **Resolved (Iteration 52).** `TargetStateCache` is now persisted via `blocklist_target_state.json` on `BlockStore` mutations. Provenance preservation (`source_node`, `BlockProvenance`) was added in Iteration 53. Stale events do not overwrite existing target-state provenance/source. See `architecture/blocklist_remove_consistency.md` and `architecture/blockstore_admin_observability.md`.
