...
| synvoid-http | Canonical HTTP parse/normalize/framing/dispatch (Phase 20; Phase 34 owns the streaming WAF body adapter) | 11084 | Yes — fail-closed framing, normalization ownership | Yes | http3 | 17 (composition surface, incl. upstream for the site→TLS adapter) | Keep | Canonical HTTP owner; wide fan-in is composition, not wrapper |
...
| synvoid-http-client | Egress HTTP client pool (+ QUIC marker; Phase 34 policy-free transport) | 2131 | Yes — pool, parity suite, PQC marker | Yes | 9 | none (leaf: hyper/rustls/moka/hyperlocal; no config/core/metrics) | Keep | Egress chokepoint; rustls-native-certs owner; site→TLS adapter in upstream, WAF body in http; eggfetch retention decided in `egress_client_decision_phase34.md` |
| synvoid-upstream | Upstream pool, balancing, health (+ site→TLS adapter, Phase 34) | 2401 | Yes — selection, health gating | Limited | 3 (http, proxy, tunnel) | config, http-client, utils | Keep | Narrow egress boundary consumed by proxy/tunnel/http |
