...
Status: Phase 35 closeout audit (Phase 33 rows re-verified; Track 4 deltas in
§Phase 31; per-crate rows recomputed 2026-09-16 from `cargo metadata
--all-features` + `wc -l` over each crate `src/`; count stays 43 `synvoid-*`
members under `crates/`). One row per workspace crate from the actual final
dependency graph (generated from each `Cargo.toml` plus root `Cargo.toml`;
LOC via `wc -l` over each crate `src/`).
...
| synvoid-utils | Shared buffers, serialization, time, strings, errors | 1826 | Yes — serialization round-trips, buffer pool ABA safety | Yes | 19 | none | Keep | Lowest-level shared crate; `buffer` feature isolates pool; Phase 35: `is_newer_version` documented approximate (not SemVer), `get_first_non_loopback_ip` documented heuristic |
| synvoid-config | All configuration types + validation | 12628 | Yes — parse/validate fail-closed, config matrix | Limited (operator-facing) | 21 | pqc | Keep | Single config owner; `dns`/`mesh` features isolate heavy deps |
...
| synvoid-http | Canonical HTTP parse/normalize/framing/dispatch (Phase 20; Phase 34 owns the streaming WAF body adapter) | 11247 | Yes — fail-closed framing, normalization ownership | Yes | http3 | 17 (composition surface, incl. upstream for the site→TLS adapter) | Keep | Canonical HTTP owner; wide fan-in is composition, not wrapper |
...
| synvoid-mesh | DHT, transport, Raft, trust, policy gates (Phase 23 authority) | 100220 | Yes — consensus, key policy, partition semantics | Limited (mesh deployments) | 6 | 13 (incl. mesh-protocol, rate-limit) | Keep | Control-plane boundary; `mesh` feature isolates openraft/proto; depends downward on `synvoid-mesh-protocol` (Phase 27); consumes shared `synvoid-rate-limit` windows, policy stays mesh-owned (Phase 33) |
| synvoid-mesh-protocol | Stable mesh wire/identity vocabulary: constants, `HybridSignature` envelope, Ed25519 `ProtocolSigner` verification, replay protection, threat taxonomy, framing (Phase 27) | 870 | Yes — wire-compat golden vectors, fail-closed framing, deterministic verification | Yes — verification-only consumers without DHT/Raft/SQLite/YARA | 1 (mesh) + root feed verification | none (leaf by design) | Keep | Dependency-isolation boundary; strict budget guard (`mesh_protocol_boundary`); `synvoid-mesh` re-exports compat paths |
| synvoid-block-store | Enforcement block state + provenance + cursors | 8548 | Yes — capacity, expiry, replay, concurrency | Limited | root only | config, core, mesh, utils, waf | Keep | Enforcement-state boundary; worker admission reads this, not TIM |
| synvoid-ipc | IPC transport + jail protocol/supervision/binary resolution (Phase 22, Phase 29 exe-dir lookup) | 12702 | Yes — framed protocol, digests, restart bounds, fail-closed routing, deterministic helper resolution | Limited | 5 (incl. jail-runtime) | config, metrics, platform, tls, utils | Keep | Trust boundary for process isolation; versioned wire protocol |
| synvoid-plugin-runtime | WASM runtime, sandbox, instance pool, manifests (+ optional native facade) | 18452 | Yes — capability gating, ABI boundary, reload prepare-commit | Limited | 4 | utils (+ optional native-extension) | Keep | Sandbox boundary; wasmtime version patch point; default graph links no shared-library loader (Phase 28) |
| synvoid-native-extension | Explicit unsafe in-process native loader + narrow backend trait + non-executing external-host seam (Phase 28) | 2410 | Yes — production/path/hash/permission gates, ABI checks, generation-aware lifetime, contract-bound validation | Limited (opt-in native deployments) | 0-1 (plugin-runtime optional; root optional) | none (leaf: axum/libloading/metrics only) | Keep | Capability-isolation boundary; `unsafe-native-extensions` feature off by default; `synvoid-mesh`/`synvoid-upload` must never depend on it |
| synvoid-jail-runtime | Child-side jail execution + dedicated binaries (Phase 29): `WasmJailService`/`YaraJailService`, sandbox-entry sequencing, `synvoid-wasm-jail` + `synvoid-yara-jail` binaries | 735 | Yes — digest re-verification, hook-only capabilities, strict sandbox ordering, wrong-kind rejection, fail-closed entry | Limited (jail deployments) | 1 (root facades/shims) | ipc, platform, plugin-runtime (wasm), yara (yara) | Keep | Process-boundary isolation; `wasm`/`yara` features separate engines per binary; never imports root/supervisor/admin/mesh |
...
| synvoid-dns | Authoritative/recursive DNS + DNSSEC protocol/transport (feature-gated; private-key custody extracted Phase 30) | 38141 | Yes — wire codec, public-only DNSSEC validation/proofs, zone lifecycle | Yes | root only | config, core, dnssec-keystore, geoip, mesh, platform, tls, utils | Keep | `dns` feature isolates hickory; `hsm` feature gates PKCS#11 via the keystore; largest optional surface |
| synvoid-dnssec-keystore | DNSSEC private-key custody: sealed generation/storage/rotation/signing/HSM (Phase 30; Phase 34 drops the last app edge) | 2710 | Yes — opaque sealed handles, 0600/atomic persistence, fail-closed HSM, KAT/rotation/permission/redaction tests | Limited (DNS deployments) | 1 (dns) | none (leaf: no synvoid-core/config; `src/time.rs` owns the clock) | Keep | Security boundary; one-way leaf (no hickory/hyper/quinn/sqlite/mesh); `pkcs11`/`hsm` features off by default so normal builds carry no `cryptoki` |
...
| synvoid-http-client | Egress HTTP client pool (+ QUIC marker; Phase 34 policy-free transport) | 1999 | Yes — pool, parity suite, PQC marker | Yes | 9 | none (leaf: hyper/rustls/moka/hyperlocal; no config/core/metrics) | Keep | Egress chokepoint; rustls-native-certs owner; site→TLS adapter in upstream, WAF body in http; eggfetch retention decided in `egress_client_decision_phase34.md` |
...
| synvoid-static-files | Static serving, minification, file manager | 6557 | Yes — path traversal prevention, caching | Limited | 3 | app-handlers, config, ipc, theme, utils | Keep | Owns minification dep tree (build isolation) |
| synvoid-upload | Upload validation + YARA policy/orchestration (facade over `synvoid-yara`) | 8242 | Yes — validation, in-jail scanning | Limited | 1 | app-handlers, config, http-client, mesh, platform, utils, yara | Keep | Upload trust boundary; engine implementation moved to `synvoid-yara` (Phase 26) |
| synvoid-yara | Canonical YARA execution boundary: engine, artifact binding, executor contract (Phase 26) | 2814 | Yes — bounded compile/scan, digest/version binding, narrow executor | Limited (upload/mesh/jail consumers) | 2 (upload, jail-runtime) + root jail/cpu | none (leaf; yara-x isolated here) | Keep | Security boundary; single `yara-x` production owner; mesh has no compiler reference |
| synvoid-platform | OS abstraction, paths, sandbox backends (Phase 32 single owner) | 5408 | Yes — platform detection, secure dirs, Landlock/Capsicum/Pledge/Job-Object/Seatbelt enforcement | Yes | 8 (incl. jail-runtime) | utils + target-gated nix, daemonize2, tokio, windows-sys, libloading, zip (never ipc/config/metrics) | Keep | Platform isolation; `macos-sandbox` experimental surface (deprecated sandbox_init) |
...
- No cycles: `synvoid-core` and `synvoid-utils` are zero-dep roots;
  `synvoid-http` has the widest fan-in (17 internal deps, Phase 34 adds
  `synvoid-upstream` for the site→TLS adapter) because it is the
...
## Phase 35 closeout deltas (2026-09-16)

Recomputed (`cargo metadata --all-features`, `cargo tree --workspace`,
`cargo tree -d`, `cargo tree -i
synvoid-platform/synvoid-rate-limit/synvoid-dnssec-keystore/synvoid-http-client`,
`cargo tree -e features`, `wc -l` over each crate `src/`). No crate created or
removed in this phase; all Phase 32-34 additions re-verified against the same
retain bar (no "keep" merely for being new):

- `synvoid-platform` (Phase 32): single compiled owner of generic process/IPC/
  socket/service/Unix/Windows/filesystem/sandbox primitives (5408 LOC, 8
  in-crate rev deps, `utils` + target-gated OS deps only). Root `src/platform/`
  is a pure alias facade (guard `platform_canonicalization_guard`). No app
  policy moved into the crate; `new()` paths byte-identical, `for_app` is the
  neutral constructor.
- `synvoid-rate-limit` (Phase 33): std-only leaf (732 LOC, 0 internal deps, 1
  in-crate rev dep `synvoid-mesh` + root WAF composition + compat re-export).
  Two real consumers: root WAF (`ratelimit/core.rs` windows + `asn_tracker.rs`)
  and mesh (`rate_limit.rs` peer limiter + `transport_types.rs` global limiter).
  Deletes the mesh window stub and the root window duplicate; blackhole/slotted/
  shm/token-bucket policy stays domain-owned. Deterministic tests (no sleeps),
  ~86ns `increment_at` hot path, no mutex/heap per event. Passes the retention
  bar on mechanism reuse + dependency isolation + deterministic invariants.
- `synvoid-dnssec-keystore` (Phase 30, Phase 34 drops the last app edge):
  one-way leaf (2710 LOC, 0 internal workspace deps, 1 rev dep `synvoid-dns`).
  No `synvoid-core`/`synvoid-config`/root edge (`src/time.rs` owns the clock);
  `pkcs11`/`hsm` off by default so normal builds carry no `cryptoki`.
- `synvoid-http-client` (Phase 34, Branch 2 retained): policy-free leaf
  transport (1999 LOC, 0 internal workspace deps, 9 in-crate rev deps).
  `synvoid-config`/`synvoid-core`/`metrics` edges removed; site→TLS lives in
  `synvoid-upstream::tls_adapter`, WAF body in `synvoid-http::streaming_waf_body`.
  Retention is a deliberate single-owner decision with an exit condition, not
  overlap: see `egress_client_decision_phase34.md` (PQ gap, closed body,
  hostname policy, UDS parity, zero dep win, maturity).
- `synvoid-upstream` (+87 LOC `tls_adapter.rs`, now 2488 LOC): narrow egress
  boundary consumed by proxy/tunnel/http (3 rev deps). `synvoid-http` (11247
  LOC, 17 internal deps) stays the HTTP composition surface; wide fan-in is
  composition, not a wrapper.
- Rejected/deferred in this closeout: moving `ResultExt`/`OptionExt`,
  `HotHashMap`/`HotHashSet`, `format_duration`, `parse_host_port`, `hash_ip`,
  or application error strings into `synvoid-utils` (single/zero consumers,
  stable-semantics bar not met — documented in `src/utils.rs`); promoting
  approximate `is_newer_version` as SemVer (documented approximate only);
  replacing the heuristic `get_first_non_loopback_ip` with a platform
  mechanism (documented heuristic); collapsing `http-client`/`upstream`/
  `tunnel` or `filter`/`app-server` (same verdict as Phase 31 — needs a scoped
  plan, not a closeout drive-by); creating any new generic HTTP crate
