...
| `ratelimit/core.rs` (`ShardedRateLimiter`, `SlottedIpRateLimiter`, `GlobalRateLimiter`, `AtomicSlidingWindow`) | 883 | none | **application composition (`keep_app_root`)** | Depends on root-only runtime: `RunningFlag`, `upstream::shared_state::SharedRateLimitTable` (typed handle; mmap IPC stays inside `synvoid-upstream`, Phase 42 `from_shared_table`), root rate-limit configs. Moving it would pull process/IPC composition into the domain crate. Consumed by `AsnTracker` and `ThreatMetricsCollector` via narrow root paths. Documented blocker. |
