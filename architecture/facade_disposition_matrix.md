...
| `http_client` | `synvoid_http_client` + root `quic_tunnel_dispatch` | facade with local adapter | 15 `crate::` + 8 `synvoid::` | No | No | `adapter_keep_app_root` | Root retains `quic_tunnel_dispatch` (depends on root tunnel/QUIC infra); `streaming_waf_body` shim now points at `synvoid-http` (Phase 34). |
