...
- Four tiers: small (4KB), medium (64KB), large (256KB), jumbo (256KB) buffers
...
- **Protocol Support:**
  ...
  - **HTTP/2:** Enabled via ALPN negotiation on server side (`src/tls/server.rs` `serve()` ALPN branch). Client-side HTTP/2 is configurable via `ProxyServer::with_http2()` builder method.
