...
| commands | CLI and supervisor command dispatch (plan + execute + typed result boundary) | keep_app_root | root app crate | typed command plan, execution layer, and supervisor-control adapter with typed outcomes/errors | Thin dispatch module; delegates to existing runtime/supervisor modules via typed adapter |
