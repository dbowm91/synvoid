...
| commands | CLI and supervisor command dispatch (plan + execute + runtime-launch boundary + typed result boundary) | keep_app_root | root app crate | typed command plan, execution layer, runtime-launch boundary, and supervisor-control adapter with typed outcomes/errors | Thin dispatch module; delegates to existing runtime/supervisor modules via typed adapters |
