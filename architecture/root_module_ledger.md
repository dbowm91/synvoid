...
| platform | Platform abstraction (OS detection, IPC, sandbox) | keep_app_root | root app crate (composition) + synvoid-platform (core + sandbox backends) | facade with local submodules — thin facade re-exports Platform/PlatformError/fs/sandbox from synvoid-platform crate (Phase 29: Landlock/Capsicum/Pledge/Job-Object/Seatbelt backends canonical in crate; root `src/platform/sandbox.rs` is a pure facade); root-owned socket/ipc/process/service modules | Duplicate fs.rs removed; Platform enum and detection re-exported from crate |
