...
| platform | Platform abstraction (OS detection, IPC, sandbox) | facade_existing_crate | synvoid-platform (all primitives + OS backends) | pure re-export facade — `src/platform/mod.rs` holds only module aliases (`ipc`, `process`, `sandbox`, `service`, `socket`, `fs`) plus historical top-level names; no root-owned socket/ipc/process/service/OS-backend code (Phase 32; guard `platform_canonicalization_guard`) | Prefer `synvoid_platform` in all new code, including root internals |
...
| sandbox | Sandbox parent policy/composition + facades (WASM/YARA jails) | keep_app_root | root app crate (parent policy) + synvoid-jail-runtime (child execution) + synvoid-ipc (protocol/supervision/binary resolution) | facade with parent composition — `policy.rs` owns `JailClient` (+ `spawn_resolved` via exe-dir lookup); `mod.rs` holds only `--wasm-jail`/`--yara-jail` forwarding shims to `synvoid-jail-runtime`; `wasm_service.rs`/`yara_service.rs` are pure re-export facades | Phase 29 packaged split; child services + sandbox entry canonical in `synvoid-jail-runtime`; spec `architecture/sandbox_jail_protocol.md` |
...
| utils | Utility types and helpers | keep_app_root | root app crate (composition) + synvoid-utils (shared) + synvoid-core (URL) | facade with local helpers — re-exports shared types from synvoid-utils crate; URL decoding re-exports synvoid-core (Phase 35, no second copy); root-only ResultExt/OptionExt/errors/HotHashMap/parse_host_port/hash_ip/format_duration with Phase 35 disposition in `src/utils.rs`; `ratelimit/` is a compat re-export over synvoid-rate-limit (Phase 33) | Duplicate ArcStr, parse_duration, timestamp functions, etc. removed; re-exported from crate |
...
| waf | WAF application composition over the canonical synvoid-waf engine | keep_app_root | root app crate (composition: WafCore/AppWaf, adapters, threat-level, rule/threat feeds, ASN, rate-limit core, traffic global) + synvoid-waf (detectors/policy/traits) + synvoid-rate-limit (shared window/contracts consumer) | facade with local composition — reusable detectors are crate-owned thin facades; WafCore is documented composition (AppWaf alias); duplicates and hot-path placeholders removed (Phase 19); sliding-window mechanism moved to synvoid-rate-limit, WAF policy (blackhole, slotted/shm counters, RateLimitDecision) stays root-owned (Phase 33) | Full WafCore move blocked: would pull GeoIP facade, tarpit handler, theme error pages, upload-validator static, traffic metrics, threat-level sqlite persistence, and worker RequestServices into the domain crate |
