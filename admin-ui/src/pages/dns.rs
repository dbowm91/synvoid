...
pub fn Dns() -> Html {
                    ...
                    Ok(config_json) => {
                        ...
                        if let Some(obj) = config_json.as_object() {
                            ...
                            if let Some(v) = obj.get("bind_addresses").and_then(|v| v.as_array()) {
                                let addrs: Vec<String> = v.iter().filter_map(|a| a.as_str().map(String::from)).collect();
                            ...
                            if let Some(v) = obj.get("forwarders").and_then(|v| v.as_array()) {
                                let forwarders: Vec<String> = v.iter().filter_map(|f| f.as_str().map(String::from)).collect();
                                config.forwarders = Some(forwarders);
                            }
                            if let Some(v) = obj.get("block_tld").and_then(|v| v.as_array()) {
                                let tlds: Vec<String> = v.iter().filter_map(|t| t.as_str().map(String::from)).collect();
    ...
    let on_save = {
        ...
        Callback::from(move |_| {
            ...
            wasm_bindgen_futures::spawn_local(async move {
                let api = ApiService::new();
                match api.update_dns_config(&serde_json::Value::Object(updated_json)).await {
                    Ok(_) => {
    ...
    let forwarders_string = edited_config_for_render.forwarders.clone()
        .map(|f| f.join("\n"))
        .unwrap_or_default();
    let block_tld_string = edited_config_for_render.block_tld.clone()
        .map(|t| t.join(", "))
        .unwrap_or_default();
