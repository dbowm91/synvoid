...
use crate::components::toast::{toast_success, toast_error};
use crate::services::ApiService;
use crate::types::{OverseerConfig, ProcessManagerConfig, SupervisorConfig, StatusResponse};
use yew::prelude::*;
        ...
        use_effect_with((), move |_| {
            ...
            wasm_bindgen_futures::spawn_local(async move {
                ...
                match api.get_process_manager_config().await {
                    Ok(resp) => {
                        if let Some(config) = resp.get("config") {
                            if let Ok(c) = serde_json::from_value::<ProcessManagerConfig>(config.clone()) {
                ...
                match api.get_supervisor_config().await {
                    Ok(resp) => {
                        if let Some(config) = resp.get("config") {
                            if let Ok(c) = serde_json::from_value::<SupervisorConfig>(config.clone()) {
                    ...
                    "overseer" => {
                        if let Some(ref config) = ov_config {
                            ...
                            match api.update_overseer_config(&payload).await {
                                Ok(resp) => {
                                    if let Ok(status) = serde_json::from_value::<StatusResponse>(resp.clone()) {
        ...
        Callback::from(move |(field, value): (String, String)| {
            if let Some(mut c) = (*overseer_config).clone() {
                match field.as_str() {
                    ...
                    "health_check_interval_secs" => c.health_check_interval_secs = value.parse().unwrap_or(5),
                    "stable_uptime_secs" => c.stable_uptime_secs = value.parse().unwrap_or(60),
                    "upgrade_validation_timeout_secs" => c.upgrade_validation_timeout_secs = value.parse().unwrap_or(10),
                    "upgrade_drain_timeout_secs" => c.upgrade_drain_timeout_secs = value.parse().unwrap_or(30),
                    "upgrade_health_check_retries" => c.upgrade_health_check_retries = value.parse().unwrap_or(5),
                    "upgrade_health_check_interval_secs" => c.upgrade_health_check_interval_secs = value.parse().unwrap_or(2),
                    "ipc_read_timeout_ms" => c.ipc_read_timeout_ms = value.parse().unwrap_or(5000),
                    "ipc_write_timeout_ms" => c.ipc_write_timeout_ms = value.parse().unwrap_or(5000),
                    "master_startup_timeout_secs" => c.master_startup_timeout_secs = value.parse().unwrap_or(30),
                    _ => {}
                }
        ...
        Callback::from(move |(field, value): (String, String)| {
            if let Some(mut c) = (*process_manager_config).clone() {
                match field.as_str() {
                    ...
                    "max_restart_attempts" => c.max_restart_attempts = value.parse().unwrap_or(5),
                    "restart_cooldown_secs" => c.restart_cooldown_secs = value.parse().unwrap_or(60),
                    "restart_backoff_max_secs" => c.restart_backoff_max_secs = value.parse().unwrap_or(300),
                    "heartbeat_timeout_secs" => c.heartbeat_timeout_secs = value.parse().unwrap_or(30),
                    "graceful_shutdown_timeout_secs" => c.graceful_shutdown_timeout_secs = value.parse().unwrap_or(30),
        ...
        Callback::from(move |(field, value): (String, String)| {
            if let Some(mut c) = (*supervisor_config).clone() {
                match field.as_str() {
                    ...
                    "scale_up_cooldown_secs" => c.scale_up_cooldown_secs = value.parse().unwrap_or(30),
                    "scale_down_cooldown_secs" => c.scale_down_cooldown_secs = value.parse().unwrap_or(60),
                    "max_restart_attempts" => c.max_restart_attempts = value.parse().unwrap_or(5),
                    "restart_cooldown_secs" => c.restart_cooldown_secs = value.parse().unwrap_or(300),
                    "health_check_interval_secs" => c.health_check_interval_secs = value.parse().unwrap_or(5),
                    "graceful_shutdown_timeout_secs" => c.graceful_shutdown_timeout_secs = value.parse().unwrap_or(30),
                    _ => {}
                }
...
fn OverseerSection(props: &OverseerSectionProps) -> Html {
    ...
    let toggle_auto_restart = props.on_change.reform(|_| ("auto_restart".to_string(), "toggle".to_string()));
