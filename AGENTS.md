...
| `crates/synvoid-yara/src/{engine,artifact,executor}.rs` (single `yara-x` owner; source-text-only, local compile, no deserializers; Phase 36) + `crates/synvoid-upload/src/` (scan policy, quarantine) | mesh/jail/upload paths (recompile approved source locally; jail service uses `engine` directly), no root `src/upload/` |
...
- **Skills**: `.opencode/skills/<name>/SKILL.md` — 48 per-subsystem guides (e.g. `dns_dnssec`, `ipc_hardening`, `raft_consensus`, `proxy_upstream`, `supervisor`, `worker_data_plane`, `config_system`, `admin_contract`, `waf_engine`, `block_store`, `tls_termination`, `plugin_runtime`, `auth`, `supply_chain`, plus `upload_security`, `yara_scanning`, `metrics_observability`, `http_client`, `challenge_pow`, `icmp_filter`). Load before working in an unfamiliar subsystem; cite canonical `crates/synvoid-*` paths for domain logic (root `keep_app_root` composition paths stay valid for wiring). Sections titled "Wave/M/P0.x fix log" inside skills are historical evidence, not TODOs — line numbers there drift, locate by symbol.
- **Config paths**: `--config-path` takes the DIRECTORY containing `main.toml` + `sites/`, not the TOML file. `--configtest` validates that directory (`<dir>/main.toml` + `<dir>/sites/*.toml`), defaulting to CWD-relative `./config/` without the flag.
