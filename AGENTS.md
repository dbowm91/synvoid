...
- **WorkerTaskRegistry**: `src/worker/task_registry.rs` — worker-level task lifecycle management with `spawn_critical()`, `spawn_background()`, `child_token()`, `shutdown_and_join()`; cancellation via `tokio::sync::watch`; bounded shutdown with configurable timeouts
- **ManagedService trait**: `src/worker/task_registry.rs` — `name()`, `shutdown()`, `join()` contract for long-lived services
- **ThreatFeedClient lifecycle**: `src/waf/threat_intel/feed_client.rs` — now uses `select!` with `shutdown_tx` watch channel; exposes `shutdown()` and `join()` methods (Iteration 61)
