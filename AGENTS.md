...
- `wasmtime` 40.0.4 arrives transitively via `synvoid-yara` → `yara-x` 1.15 (YARA boundary only, not the WASM sandbox); direct wasmtime is 42.0.2 via `[patch.crates-io]`. 42.0.2 is AFFECTED by RUSTSEC-2026-0269 (`wasmtime-wasi` unreachable — capability absence, not a patch); ≥46.0.3 upgrade blocked by a bumpalo conflict. 16 advisory ignores in `deny.toml` (mirrored in `.cargo/audit.toml`), expiring against current UTC date. See `architecture/dependency_security_baseline_phase25.md`. Never call 42.0.2 "patched" for 0269.
