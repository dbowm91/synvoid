...
| `PeerSessionTask` | `crates/synvoid-mesh/src/mesh/transport.rs` (Iteration 72) |
...
- **AuxiliaryTask**: `crates/synvoid-mesh/src/mesh/lifecycle.rs` — transport-owned auxiliary task (Iteration 73). `AuxiliaryTaskKind::PreflightRoute` for steady-state preflight. Tracked in `auxiliary_tasks: HashMap<MeshTaskId, AuxiliaryTask>`, aborted and awaited during shutdown/recovery
...
- **PeerSessionExit**: `crates/synvoid-mesh/src/mesh/lifecycle.rs` — session exit metadata with `session_id`, `node_id`, `reason: PeerSessionExitReason`, `generation: u64` (Iteration 73). Generation counter prevents stale completions from removing newer entries
