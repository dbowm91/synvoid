...
- **WorkerMeshAttachment**: `src/worker/unified_server/mesh_attachment.rs` — worker-side mesh attachment orchestration; `attach_mesh()` creates supervision pipeline, handles required/optional mesh startup, registers support tasks after mesh succeeds, and returns typed `MeshStartupState` to startup/supervision/shutdown. Extracted from `startup_plan.rs` in Iteration 95.
