...
| `src/worker/unified_server.rs` (monolithic) | `src/worker/unified_server/` (split 2026-06) — see `state.rs`, `services.rs`, `init_apps.rs`, `init_waf.rs`, `init_mesh.rs`, `init_runtime.rs`, `init_config.rs`, `lifecycle.rs`, `passthrough_validation.rs`, `startup_plan.rs`, `supervision_loop.rs`, `shutdown_executor.rs`, `supervisor_notify.rs` |
...
| `src/worker/unified_server/passthrough_validation.rs` | TLS passthrough classification and validation — pure functions (no I/O, no side effects); extracted from mod.rs |
...
| `src/main.rs` (command dispatch) | `src/commands/plan.rs` + `src/commands/execute.rs` + `src/commands/runtime_launch.rs` (Iteration 101: extracted typed command planning and execution; Iteration 102: typed restart pre-action, distinct hash-token error; Iteration 103: typed supervisor-control result boundary in `src/commands/supervisor_control.rs`; Iteration 104: handler output/data separation — data-bearing outcomes, centralized formatting, real threat-feed metadata; Iteration 106: runtime launch delegated to `runtime_launch.rs` — `RuntimeLaunchContext` (structured inputs), `RuntimeLaunchPlan` (pure per-mode description), `RuntimeLaunchOutcome` (typed result), `plan_runtime_launch()` (pure planner), `execute_runtime_launch()` (side-effecting launcher); Iteration 107: one-shot commands delegated to `one_shot.rs` — `OneShotOutcome`/`OneShotError` typed boundary, `execute_one_shot_command()` adapter, centralized formatting and exit codes) |
...
### Module Key Facts
- **Command Dispatch**: `src/commands/mod.rs` — typed command classification (`SynvoidCommandPlan`) and execution; `src/commands/plan.rs` for planning, `src/commands/execute.rs` for execution, `src/commands/runtime_launch.rs` for runtime-launch boundary, `src/commands/one_shot.rs` for one-shot command boundary. `src/main.rs` is a thin entrypoint that delegates here. `--restart` is a typed `CommandPreAction::RestartSupervisor` that preserves control address and TLS; not a dead `SupervisorControlCommand` variant. `SupervisorControlOutcome` carries data-bearing variants (`Status(SupervisorStatusDisplay)`, `Stop(StopOutcome)`, `Rehash(RehashOutcome)`, `ThreatFeedExported(ThreatFeedExportSummary)`). Formatting is centralized in `display()`. Handlers expose `_data` variants returning structured data. **Iteration 105**: `SupervisorControlError` has actionable typed variants (`ConnectionUnavailable`, `Timeout`, `Protocol`, `RequestRejected`, `Authentication`, `UnsupportedFeature`, `Io`, `InvalidResponse`, `Unknown`); `classify_control_error()` replaces the old `boxed_error_to_control_error` catch-all. All variants return exit code 1 for backwards compatibility. **Iteration 106**: Runtime launch delegated to `runtime_launch.rs` — `RuntimeLaunchContext` (structured inputs), `RuntimeLaunchPlan` (pure per-mode description), `RuntimeLaunchOutcome` (typed result), `plan_runtime_launch()` (pure planner), `execute_runtime_launch()` (side-effecting launcher). `execute.rs` no longer builds Tokio runtimes or worker args directly. **Iteration 107**: One-shot commands delegated to `one_shot.rs` — `OneShotOutcome`/`OneShotError` typed boundary, `execute_one_shot_command()` adapter, centralized formatting and exit codes. `execute.rs` no longer owns one-shot implementation details.
- **Iteration 108**: Final CLI surface audit — precedence rules, feature-gate matrix, exit-code model, and manual compatibility checklist documented in `architecture/cli_supervisor_command_dispatch.md`. 18 new planner tests covering precedence, cost clamping, mesh gates, and worker-mode mutual exclusion.
- **Iteration 109**: Output compatibility lock — script-facing stdout contracts documented in `architecture/cli_supervisor_command_dispatch.md`. JSON export outputs guaranteed JSON-only (no preamble). Token/hash outputs guaranteed label-free single-line. Regex safe/unsafe output and exit codes tested. `generatenewtoken` first-line-token behavior documented and tested. Source guards prevent human-preamble text in JSON display outputs and labeled token/hash output. Output contract tests in `src/commands/one_shot.rs`; guard tests in `tests/cli_command_dispatch_guard.rs`.
...
- **DataPlaneServicesBuilder**: `src/worker/unified_server/services.rs` - data-plane service assembly boundary; requires `serverless_manager` in constructor; `build_and_cross_wire()` centralizes post-build cross-wiring (threat-intel policy application, mesh service wiring); under mesh, `DataPlaneServices` carries optional `ThreatIntelPolicyContext` and a low-risk apply helper; a root-side helper can build the context from explicit handles; bootstrap leaves canonical as `None`; the Supervisor's `CanonicalTrustSnapshot` arrives via IPC after bootstrap and is applied through `update_threat_intel_policy_context()`; no global fallback in builder. **RequestServices** (`src/worker/context.rs`) is the narrow request-path handle — must not import worker startup/supervision/shutdown modules. Iteration 98: field ownership documented, centralized cross-wiring via `build_and_cross_wire()`, boundary guards enforce service boundary invariants
- **Http3RequestMetadata**: `crates/synvoid-http/src/http3_request_dispatch.rs` — groups per-request metadata (start, route_result, path, method, headers, host, query_string, user_agent, client_ip) for HTTP/3 dispatch. Implements `From<&Http3RequestPrelude>`.
- **Http3DispatchDeps**: `crates/synvoid-http/src/http3_request_dispatch.rs` — groups dependency/service handles (max_request_size, streaming_waf scanners, connection_limiter, main_config, client, upstream_client_registry, bandwidth, metrics) for HTTP/3 dispatch. Replaces the previous 21-parameter function signature.
- **Request Pipeline Stage Vocabulary**: Both HTTP/1 and HTTP/3 pipelines now use shared stage vocabulary documented in `architecture/http_request_pipeline.md`. Module-level doc comments in `http3_request_dispatch.rs` and `http_request_flow.rs` map each protocol's stages to the shared vocabulary.
...
- **WorkerMeshAttachment**: `src/worker/unified_server/mesh_attachment.rs` — worker-side mesh attachment orchestration; `attach_mesh()` is a thin orchestration wrapper delegating to `create_mesh_pipeline()`, `start_required_mesh()`, and `await_optional_mesh_startup()` helpers. Each helper owns one piece of behavior (pipeline creation, required startup, optional startup race loop) and returns typed outputs. Extracted from `startup_plan.rs` in Iteration 95, polished in Iteration 96, ordering corrected in Iteration 97.
...
| HTTP request pipeline | `http_request_pipeline.md` | — |
