...
- **SupervisionOutcome**: `src/worker/task_registry.rs` — typed enum (`Lifecycle { event, accepted }` | `DirectCause(WorkerShutdownCause)`) returned by the supervision loop, preserving direct shutdown causes without converting to fake lifecycle events
