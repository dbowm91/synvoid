...
| `StagedPeerResource` | `crates/synvoid-mesh/src/mesh/lifecycle.rs` (Iteration 71) |
...
- **StagedPeerResource**: `crates/synvoid-mesh/src/mesh/lifecycle.rs` — tracks exact peer mutations during startup: `session_id`, `node_id`, `topology_existed_before`, `connection_inserted`, `session_task_created` (Iteration 71)
...
- **MeshAcceptLoopReport**: `crates/synvoid-mesh/src/mesh/lifecycle.rs` — declared with `drained_handshakes`, `aborted_handshakes`, `rejected_at_capacity` fields. Declared but not yet wired into the accept loop (deferred to a future iteration) (Iteration 70)
