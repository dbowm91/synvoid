...
- **Mesh trust domains**: `architecture/mesh_trust_domains.md` — design note (iteration 7) classifying modules by trust domain (transport, advisory_dht, canonical, identity, policy, services, compat); advisory DHT vs. canonical Raft distinction; policy as decision layer; `RECORD_STORE_GLOBAL` explicitly called out as compat/legacy; includes invariants, target module shape, import rules, review checklist, and `CanonicalTrustReader` as preferred first seam. **Iteration 8 implemented**: `crates/synvoid-mesh/src/mesh/canonical.rs` (CanonicalFreshness/Decision/Reason, CanonicalTrustReader trait, StaticCanonicalTrustReader + SnapshotCanonicalTrustReader over EdgeReplicaManager); narrow `get_revoked_node` helper; 17 offline tests; low-risk `dyn CanonicalTrustReader` consumer example; see plan `plans/mesh_canonical_trust_reader_iteration_8.md`. **Iteration 9 consumer migration**: `peer_auth.rs` now has low-churn `validate_peer_canonical_status(&dyn CanonicalTrustReader, ...)` helper for the canonical revocation + global-auth subset of role checks (freshness semantics tightened, `node_revocation_status` added for clarity, real snapshot age from replica metadata, focused Static-only tests); production call sites and old paths untouched. See plan `plans/mesh_canonical_reader_consumer_iteration_9.md` and `architecture/mesh_trust_domains.md`. **Iteration 10 test hardening**: precise rustdoc on the helper (scope, what it does/does not validate, freshness, `is_global() && !is_origin()` global-role guard and `GLOBAL_ORIGIN` exemption rationale); expanded StaticCanonicalTrustReader tests covering authorized/unauthorized/revoked (all roles + composites), unavailable (permissive for revocation, fail-closed for global auth), stale (explicit, with future-policy note), edge non-global, and `GLOBAL_ORIGIN` exemption case. Global-role semantics (`GLOBAL`/`GLOBAL_EDGE` require canonical global auth in helper; origin-carrying composites exempt here) reviewed and kept intentional + explicitly tested. No rename (docs+tests suffice); no production rewiring; no module moves. See `architecture/mesh_trust_domains.md` (updated with Iteration 10 note) and plan `plans/mesh_peer_auth_canonical_tests_iteration_10.md`. See `CanonicalTrustReader` in `crates/synvoid-mesh/src/mesh/canonical.rs` (Iteration 8) and `architecture/mesh_trust_domains.md`.
