...
| `crates/synvoid-upstream/src/tls_adapter.rs` (`upstream_tls_from_site_config` site→TLS) + `crates/synvoid-http/src/streaming_waf_body.rs` (`StreamingWafBody`; scanner from `synvoid_core::streaming_waf` via `synvoid_http::shared_handler`) (Phase 34) | `crates/synvoid-http-client/` (generic transport only: no `synvoid-config`/`synvoid-core`/`metrics` edges), `src/http_client/streaming_waf_body.rs` (shim now points at `synvoid-http`) |
...
- `wasmtime` 40.0.4 arrives transitively via `synvoid-yara` → `yara-x` 1.15 (YARA boundary only, not the WASM sandbox); direct wasmtime is 36.0.15 LTS from crates.io (supported through 2027-08-20, no git patch — Phase 37 removed the 42.0.2 `[patch.crates-io]` entry and the `allow-git` exception). 36.0.15 is PATCHED for RUSTSEC-2026-0269 (>=36.0.14) and unaffected by the 2026-04 advisories (proven by a clean isolated `cargo audit` with no ignores); the 0269 ignore is retained solely for transitive 40.0.4. Phase 39 removed the minifier side of the old `bumpalo` conflict (vendored `third-party/minify-html-compat` fork: exact 0.18.1 `src/` + manifest-only Oxc 0.95→0.111 via root `[patch.crates-io]`, no git source, `minify_fork_is_temporary_guard`-enforced; `oxc_allocator` now 0.111.0 with no `minify-html → bumpalo` edge). The yara-x >=1.19 upgrade itself is now Phase 40 work (`linkme` feature removal + lock float including the wasm-bindgen chain — resolver proven past the old conflict in a disposable probe). 16 advisory ignores in `deny.toml` (mirrored in `.cargo/audit.toml`), expiring against current UTC date. See `architecture/dependency_security_baseline_phase25.md` (§10 for Phase 39). Never call transitive 40.0.4 "patched" for 0269.
- Phase 36 (YARA deserialization exposure closure, 2026-09-17): source text is now the only executable YARA input — `reload_with_compiled_rules`, `CompiledArtifact::{deserialize_verified,from_bytes_with_binding}`, mesh `local_compiled_rules`/`apply_compiled_rules`/`get_current_compiled_rules`, and `YaraRuleSourceType::CompiledBundle` are removed; upload/mesh/jail recompile approved source locally. GHSA-2jx3-ff3v-j7jj (yara-x <=1.18) has no remote path to any deserializer after this closure. The yara-x >=1.19 engine upgrade itself is Phase 40 work (Phase 39 cleared the `bumpalo` conflict: `wasmtime` >=43 needs `bumpalo ^3.20.0` vs old `minify-html` 0.18.1 → `oxc_allocator` 0.95.0 `=3.19.0` — that pin is gone; remaining items are dropping the removed `linkme` feature and floating the lock) — `YARA_ENGINE_VERSION` stays `yara-x/1.15` until it lands. Never add an advisory ignore for the GHSA to pretend 1.15 is fine; never call 1.15 "patched".
