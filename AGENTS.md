...
- **MeshStartupStage**: `crates/synvoid-mesh/src/mesh/lifecycle.rs` — tracks all resources created during a single startup attempt. On success, `commit_startup()` transfers ownership to `MeshTransport`. On failure, `rollback_startup()` cleans up all staged resources (Iteration 70)
