...
- **MeshGenerationSupport**: `src/worker/unified_server/mod.rs` — worker-owned support-generation type with `generation`, `task_ids`, and `cancel_tx`. `register_mesh_generation_support()` returns `Result<MeshGenerationSupport, WorkerShutdownCause>`. Generation-specific cancellation via watch channel (Iteration 87)
