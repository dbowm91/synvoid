...
- **Supervision pipeline**: Uses `state.mesh_status.clone()` (single authoritative allocation). Coordinator applies event transitions before policy decisions. Readiness gate checks `allow_degraded_readiness`
