...
- **MeshAcceptLoopReport**: `crates/synvoid-mesh/src/mesh/lifecycle.rs` — wired into mesh accept loop (Iteration 71). Accept loop tracks `drained_handshakes` and `aborted_handshakes` during shutdown; `rejected_at_capacity` remains untracked (always zero). Report stored in `MeshTransport::accept_loop_report`, read by `shutdown_with_timeout()` to populate `MeshShutdownReport`.
