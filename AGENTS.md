...
- **StagedPeerResource**: `crates/synvoid-mesh/src/mesh/lifecycle.rs` — tracks exact peer mutations during startup: `session_id`, `node_id`, `topology_existed_before`, `connection_inserted`, `session_task_created`, `dht_registration_created` (Iterations 71–72)
...
- **MeshTransport::recover_failed_state()**: `crates/synvoid-mesh/src/mesh/transport.rs` — recovery from `Failed` state (Iteration 72). Acquires lifecycle lock, re-runs cleanup, verifies no owned resources remain, transitions to `Stopped`. `can_start()` does NOT allow `Failed` — this is the only recovery path.
